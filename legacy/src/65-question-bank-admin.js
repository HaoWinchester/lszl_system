      ...
      return `
        <button type="button" class="qb-list-item paper ${p.id === state.selectedPaperId ? 'active' : ''}" data-paper-id="${escapeHTML(p.id)}">
          ...
          <span class="qb-paper-text">
            ...
            <small>${escapeHTML(p.subject)} · ${count}/${p.totalCount || 180} 题 · ${escapeHTML(status)}</small>
          </span>
    ...
    if(meta){
      if(!paper){
        ...
        meta.innerHTML = `
          ...
          <span class="qb-badge">已组 ${count} 题</span>
          <span class="qb-badge">目标 ${paper.totalCount || 180} 题</span>
