...
def choose_creator_and_load(page: Page, question_id: str, draft_title: str) -> str:
    ...
    page.wait_for_function("() => Boolean(prepRuntime.serverActor)")
...
with sync_playwright() as playwright:
    ...
    try:
        ...
        draft_id = choose_creator_and_load(
            offline_page, question_ids[2], f"离线恢复草稿 {stamp}"
        )
        ...
        save_error = offline_page.evaluate(
            """async () => {
              try { await window.PMPPrepDraftUi.save(); return '' }
        ...
        offline_page.evaluate("() => window.PMPPrepDraftUi.save()")
        offline_page.wait_for_function("() => prepRuntime.dirty === false")
        saved_draft = assert_ok(
            owner.request.get(BASE + f"/api/v1/content-prep/drafts/{draft_id}"),
            "read recovered shared draft",
        )["draft"]
        assert any(
        ...
        for draft_id in reversed(draft_ids):
            owner.request.delete(BASE + f"/api/v1/content-prep/drafts/{draft_id}")
        if bank_id:
