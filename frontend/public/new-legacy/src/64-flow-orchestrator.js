...
(function(global){
  ...
  function useSession(session,reason='restore'){
    ...
    if(!publishingSessionChange){
      ...
      try{try{global.dispatchEvent(new CustomEvent('kg:learning-session-changed',{detail:{reason,session:clone(active)}}))}catch(e){}}finally{publishingSessionChange=false}
    }
    return clone(active);
  }
