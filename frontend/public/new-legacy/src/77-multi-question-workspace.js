...
(function(global){
  ...
  const state={
    ...
    answerSelections:new Map(),matchingDrafts:new Map(),
  ...
  function lightweightPaperEntry(row={}){
    const paperId=String(row.paperId||row.id||'');
    const releaseId=String(row.releaseId||'');
    const configuredCount=Math.max(0,Number(row.totalCount||row.questionCount||0));
    return {
      ok:true,
      paper:{...row,id:paperId,paperId,releaseId,status:'published',availability:'published'},
      items:[],configuredCount,targetCount:configuredCount,availableCount:configuredCount,
      missingCount:0,damagedCount:0,blockedCount:0,issues:[]
    };
  }
  async function loadPublishedPapers(target={}){
    const repository=global.KGPublishedPaperRepository;
    const resolver=global.KGPublishedQuestionResolver;
    await repository?.ready?.();
    const options={respectRole:true,mode:'multi_question_canvas'};
    ...
    const catalog=rows.map(row=>{
      const releaseId=String(row.releaseId||'');
      if(!releaseId||seen.has(releaseId))return null;
      seen.add(releaseId);
      return repository?.peekResolved?.(releaseId)||lightweightPaperEntry(row);
    }).filter(Boolean);
    const saved=readPaperSelection();
    const requestedPaperId=String(target.paperId||state.paperId||saved.paperId||'');
    const requestedReleaseId=String(target.releaseId||state.releaseId||saved.releaseId||'');
    let selected=catalog.find(entry=>requestedReleaseId&&String(entry.paper?.releaseId||'')===requestedReleaseId)
      ||catalog.find(entry=>requestedPaperId&&String(entry.paper?.id||'')===requestedPaperId)
      ||catalog[0]||null;
    const identifier={paperId:String(selected?.paper?.id||requestedPaperId),releaseId:String(selected?.paper?.releaseId||requestedReleaseId)};
    if((identifier.paperId||identifier.releaseId)&&typeof resolver?.resolvePaper==='function'){
      const cached=identifier.releaseId?repository?.peekResolved?.(identifier.releaseId):null;
      const resolved=cached||await resolver.resolvePaper({...identifier,mode:'multi_question_canvas'},options);
      if(!resolved?.ok){
        const message=resolver.message?.(resolved,'指定发布版本不可用。')||resolved?.message||'指定发布版本不可用。';
        const error=new Error(message);error.result=resolved;throw error;
      }
      selected=resolved;
      const releaseId=String(resolved.paper?.releaseId||identifier.releaseId);
    ...
    return {
      ...
      releaseId:String(selected?.paper?.releaseId||'')
    };
  }
  ...
  async function rebuildQuestionSources(options={}){
    ...
    if(showLoading)global.KGLearningLoading?.show?.({title:'正在加载试卷',message:'正在读取试题…'});
    renderPaperSelector();
    try{
      invalidateQuestionSources();
      const loaded=await loadPublishedPapers(options);
      state.paperCatalog=loaded.catalog;
      state.papers=loaded.catalog.map(entry=>entry.paper);
      ...
      state.paperCatalog=previous.paperCatalog;state.papers=previous.papers;state.paperId=previous.paperId;state.releaseId=previous.releaseId;state.paperStats=previous.paperStats;state.questions=previous.questions;
      state.questionLoadError=String(error?.message||error||'发布试卷读取失败');
      console.error('多题画布发布试卷读取失败',error);
      if(showLoading)notify('试卷读取失败，请稍后重试。');
      if(byId('qwQuestionDrawer')?.classList.contains('open'))renderQuestionDock();
      return false;
    }finally{
      questionSourcesLoading=false;
      if(showLoading)global.KGLearningLoading?.hide?.();
      renderPaperSelector();
    }
  }
  ...
  function renderPaperSelector(){
    ...
    select.disabled=questionSourcesLoading;
  ...
  function buildQuestionList(){
    ...
    // 发布试卷目录未就绪（细粒度 API 异步载入）时先触发载入，载入完成后 rebuild 会重建列表
    if(!state.paperCatalog.length){void rebuildQuestionSources();return}
    const saved=readPaperSelection();
  ...
  function practiceAnswerPayload(record,key){
    ...
    return {
      ...
      ...(question.type==='matching'?{selectedPairs:key}:{selectedAnswer:String(key||'')})
    };
  }
  function markQuestionCompleted(record,result={}){
    const node=record?.node||{},question=resolvedQuestionForNode(node)||{};
    const context={
      paperId:String(node.paperId||question.sourcePaperId||state.paperId||''),
      releaseId:String(node.releaseId||question.sourceReleaseId||state.releaseId||''),
      questionId:String(node.questionId||question.id||question.sourceQuestionId||''),
      bankId:String(node.bankId||question.sourceBankId||''),
      mode:'practice_mode'
    };
    const sessions=global.KGLearningSessionStore,userId=sessions?.currentUserId?.()||'guest';
    let session=sessions?.get?.(context,userId)||sessions?.ensure?.(context,{userId,mode:'practice_mode'});
    if(session){
      const completedAt=result?.completion?.completedAt||session.completedAt||Date.now();
      session=sessions.save?.({...session,status:'completed',completedAt,currentStep:Math.max(1,Number(session.currentStep||1)),answer:{...(session.answer||{}),selectedOptionId:String(result?.completion?.selectedAnswer||''),...(result?.completion?.selectedPairs?{selectedPairs:result.completion.selectedPairs}:{}),submitted:true,isCorrect:Boolean(result?.correct)}})||session;
    }
    renderQuestionDock();
    return session;
  }
  ...
  async function submitPracticeAnswer(record,key,options={}){
    ...
    const question=resolvedQuestionForNode(record.node)||{},matching=question.type==='matching';
    if(matching&&!global.KGQuestionAnswerSet?.pairs(question,key,true)){notify('请先完成全部配对。');return false;}
    if(!matching)key=String(key||'');
    ...
    if(matching){payload.selectedPairs=key;delete payload.selectedAnswer;}else payload.selectedAnswer=key;
    const persistent=Boolean(options.persistent);
    const correct=matching?global.KGQuestionAnswerSet.gradePairs(question,key):localAnswerCorrect(record,key);
    ...
    const result={correct,completion:{status:'completed',...(matching?{selectedPairs:key}:{selectedAnswer:key}),correct,completedAt:Date.now()}};
    markQuestionCompleted(record,result);
    const practice=recordPracticeAttempt(record,key,'',correct);
    refreshSingleCardMarkup(record);
    if(!matching)flashOption(record,key,correct?'is-correct-flash':'is-wrong-flash',correct?CORRECT_FLASH_DURATION:WRONG_FLASH_DURATION);
    if(persistent&&correct&&!matching)setPersistentCorrectAnswer(record,key);
  ...
  function blankSynthesisDraft(questions=[]){
    return {
      valid:true,
      fallbackReason:'所选题目没有共同且唯一的启用原则预设。',
      synthesisType:'principle',
      cardType:'user',
      principleId:'',
      sourcePresetId:'',
      presetVersion:0,
      title:'我的归纳（'+questions.length+' 题）',
      principleTag:'',
      content:'',
      tags:[],
      color:'#ede9fe',
  ...
  async function quickCreateSynthesis(){
    if(!canEdit())return false;
    const records=selectedRecords();
    const questions=records.filter(record=>record.node?.nodeType==='question-reference');
    if(questions.length<2){notify('请先框选至少 2 张题目卡再生成归纳。');return false}
    let payload=synthesisDraftFromSelection(records);
    if(!payload.valid){notify(payload.error||'无法按正确选项原则生成归纳卡。');return false}
    if(payload.cardType==='user'){
      if(typeof PersonalCards.create!=='function'){notify('个人归纳卡服务暂不可用，请刷新后重试。');return false}
      try{
        const card=await PersonalCards.create(personalCardInput(payload));
        payload={...payload,...personalCardNodeFields(card),cardType:'user',autoGenerated:false};
      }catch(error){notify('个人归纳卡创建失败：'+String(error?.message||'请稍后重试'));return false}
    }
  ...
  function renderCards(){
    ...
    for(const record of state.cards.values())bindMixedCard(record);
  ...
  function renderQuestionDock(){
    ...
    list.innerHTML=filtered.length?filtered.map((item,visibleIndex)=>{
      ...
      const checked=state.selectedQuestionKeys.has(questionSelectionKey(item));
      const view=questionDisplayView(question);
      return '<div class="qw-question-item '+(existing?'in-workspace':'')+(checked?' is-question-selected':'')+'" data-question-index="'+state.questions.indexOf(item)+'" data-question-id="'+escapeHTML(stableQuestionId)+'" data-bank-id="'+escapeHTML(stableBankId)+'" data-paper-id="'+escapeHTML(stablePaperId)+'" data-release-id="'+escapeHTML(stableReleaseId)+'" draggable="'+(!state.readonly)+'">'
        +'<label class="qw-question-select" title="选择后可整批拖入画布"><input type="checkbox" data-qw-question-select="'+state.questions.indexOf(item)+'" aria-label="选择第 '+String(visibleIndex+1)+' 题"'+(checked?' checked':'')+'><span aria-hidden="true"></span></label>'
  ...
  function batchDragPayload(item){
    const items=state.selectedQuestionKeys.has(questionSelectionKey(item))?selectedQuestionItems():[item];
    return {kind:'question-batch',items:items.map(source=>({
      questionId:String(source.question?.id||source.question?.sourceQuestionId||''),
      bankId:String(source.bank?.id||source.question?.sourceBankId||''),
      paperId:String(source.paper?.id||source.question?.sourcePaperId||state.paperId||''),
      releaseId:String(source.paper?.releaseId||source.question?.sourceReleaseId||'')
    }))};
  }
  function batchQuestionPositions(count,anchor={}){
    const columns=Math.min(3,Math.max(1,Number(count||0))),width=430,height=420,gapX=34,gapY=42;
    return Array.from({length:Math.max(0,Number(count||0))},(_,index)=>({
      x:Number(anchor.x||0)+(index%columns)*(width+gapX),
      y:Number(anchor.y||0)+Math.floor(index/columns)*(height+gapY),width,height
    }));
  }
  function addQuestionItems(items,anchor){
    if(!canEdit())return {created:[],skipped:0};
    const ordered=(Array.isArray(items)?items:[]).filter(item=>item?.question);
    const addable=ordered.filter(item=>!currentNodeByQuestion(item.question,item.bank));
    const skipped=ordered.length-addable.length;
    if(!addable.length){notify('所选 '+skipped+' 道题已在当前画布中，未重复添加。');return {created:[],skipped}}
    const before=workspaceSnapshot(),positions=batchQuestionPositions(addable.length,anchor||defaultNodePosition()),created=[];
    state.suppressStoreEvent=true;
    try{
      addable.forEach((item,index)=>{
        const preferred=findOpenCardPosition(positions[index],{width:430,height:420});
        const result=store()?.addQuestionReference?.(
          ...
          String(item.bank?.id||item.question.sourceBankId||''),preferred,workspaceOptions()
        );
        if(result?.created&&result.node)created.push(String(result.node.id));
      });
    }finally{state.suppressStoreEvent=false}
    state.workspace=store()?.ensure?.(workspaceOptions())||state.workspace;
    const after=workspaceSnapshot();
    pushWorkspaceHistory('批量加入题目',before,after);
    renderCards();
    if(created.length){setCardSelection(created,{reason:'question-batch-added'});focusNode(created[0],{zoom:Math.max(state.zoom,.62)})}
  ...
  function bindMixedCard(record){
    global.KGQuestionMaterials?.bindMedia(record.element);
    const container=record.element.querySelector('[data-qw-matching]');if(!container)return;
    const question=resolvedQuestionForNode(record.node)||{},selected=state.answerSelections.get(String(record.id)),pairs=selected&&typeof selected==='object'?selected:state.matchingDrafts.get(String(record.id))||{};
    const button=record.element.querySelector('[data-qw-confirm-matching]');button.hidden=!!selected;button.disabled=!global.KGQuestionAnswerSet?.pairs(question,pairs,true);
    global.KGQuestionMaterials?.bind(container,{question,selectedPairs:pairs,readOnly:!!selected,reveal:!!selected,onChange:value=>{state.matchingDrafts.set(String(record.id),value);button.disabled=!global.KGQuestionAnswerSet.pairs(question,value,true);}});
    button.onclick=()=>submitPracticeAnswer(record,state.matchingDrafts.get(String(record.id))||{});
  }
  ...
  function questionDragStart(event){
    ...
    state.dragPayload=batchDragPayload(item);
    ...
    if(transfer){
      for(const type of [MIME,'text/x-kg-question-reference','text/plain']){
        try{
          ...
          if(parsed?.kind==='question-batch'&&Array.isArray(parsed.items))return parsed;
          if(parsed?.questionId)return {kind:'question-batch',items:[parsed]};
        }catch(e){}
      }
    }
  ...
  function dropQuestion(event){
    ...
    const items=(payload.items||[]).map(reference=>state.questions.find(candidate=>
      String(candidate.question?.id||candidate.question?.sourceQuestionId||'')===String(reference.questionId||'')
      &&String(candidate.bank?.id||candidate.question?.sourceBankId||'')===String(reference.bankId||'')
      &&(!reference.paperId||String(candidate.paper?.id||candidate.question?.sourcePaperId||'')===String(reference.paperId))
      &&(!reference.releaseId||String(candidate.paper?.releaseId||candidate.question?.sourceReleaseId||'')===String(reference.releaseId))
    )).filter(Boolean);
    if(!items.length){
      notify('这些题不在当前已发布试卷中，请重新打开题目库。');
      return;
    }
    const point=clientToWorld(event.clientX,event.clientY);
    addQuestionItems(items,{x:point.x-215,y:point.y-120,width:430,height:420});
  }
  ...
  function bind(){
    ...
    byId('qwPaperSelect')?.addEventListener('change',async event=>{
      if(questionSourcesLoading){renderPaperSelector();return}
      ...
      const changed=await rebuildQuestionSources({paperId:String(entry?.paper?.id||''),releaseId:String(entry?.paper?.releaseId||''),showLoading:true});
      if(changed){state.selectedQuestionKeys.clear();clearPaperTransientState();savePaperSelection();renderQuestionDock()}
    });
    ...
    byId('qwQuestionList')?.addEventListener('click',event=>{
      if(event.target.closest?.('[data-qw-question-select]'))return;
    ...
    byId('qwQuestionList')?.addEventListener('change',event=>{
      const checkbox=event.target.closest?.('[data-qw-question-select]');
      if(!checkbox)return;
      const item=resolveQuestionItem(checkbox);
      if(!item)return;
      const key=questionSelectionKey(item);
      if(checkbox.checked)state.selectedQuestionKeys.add(key);else state.selectedQuestionKeys.delete(key);
      renderQuestionDock();
    });
  ...
  async function init(){
    ...
    void rebuildQuestionSources({showLoading:true});
