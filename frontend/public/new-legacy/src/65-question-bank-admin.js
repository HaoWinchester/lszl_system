...
(function(){
  ...
  const AnswerSet=window.KGQuestionAnswerSet||{};
  ...
  function readPreference(key,fallback=null){try{return Preferences?.getJSON?.(key,fallback)??fallback}catch(error){return fallback}}
  function writePreference(key,value){try{Preferences?.setJSON?.(key,value);return true}catch(error){return false}}

  const TeacherDomains=window.KGTeacherDomains||{};
  const Difficulty=window.KGDifficultyService||TeacherDomains.DifficultyService||{};
  // Lifecycle and audit compatibility contracts delegated to src/teacher modules.
  ...
  function teacherDomainServices(){
    ...
    const safeDelete=TeacherDomains.QuestionBank?.SafeDeleteService?.create?.({
      references:()=>({papers:state.papers,releases:window.KGPaperReleaseApi?.catalog?.()||[]}),audit,actor:currentActor
    })||null;
    ...
    const paperPicker=TeacherDomains.PaperManagement?.PaperQuestionPicker?.create?.({lookup:paperQuestionLookup,groupMembers:(id,bankId)=>paperCandidates(currentPaper()?.subject||'PMP').filter(row=>row.bank.id===bankId&&(row.question.caseGroup||row.question.metadata?.caseGroup)?.id===id).sort((a,b)=>(a.question.caseGroup||a.question.metadata.caseGroup).order-(b.question.caseGroup||b.question.metadata.caseGroup).order).map(row=>({bankId:row.bank.id,questionId:row.question.id,score:1}))})||null;
  ...
  function canonicalTagName(value){
    ...
    return window.KGQuestionClassification?.canonicalTagName?.(current)||current;
  }
  ...
  function scopeLabel(){
    ...
    return '当前空间：' + decodeURIComponent(scope.replace(/^user__/, '')) + ' 的服务器题库';
  }
  ...
  function multipleChoiceTemplateQuestion(){
    return normalizeQuestion({
      ...emptyQuestion('PMP'),
      id:'question-template-multiple-choice',
      title:'多选题导入示例',
      type:'multiple_choice',
      stem:'以下哪些做法有助于管理项目风险？',
      options:[
        {id:'A',text:'识别并登记风险'},
        {id:'B',text:'忽略低概率风险'},
        {id:'C',text:'规划并跟踪风险应对'},
  ...
  function normalizeQuestion(question, index=0){
    ...
    const type=String(question.type || 'single_choice');
    const correct = ['matching','multiple_choice'].includes(type) ? '' : String(question.correctAnswer || '');
    ...
    const correctOptionIds=type==='multiple_choice'
      ? (AnswerSet.correctIds?.({...question,options}) || options.filter(option=>option.correct).map(option=>option.id))
      : [];
    const correctAnswer = ['matching','multiple_choice'].includes(type) ? '' : String(question.correctAnswer || detectedCorrect?.id || options[0]?.id || '');
    options.forEach(o => { o.correct = type==='multiple_choice' ? correctOptionIds.includes(o.id) : (o.id === correctAnswer || o.correct && !correctAnswer); });
    ...
    return {
      ...question,...Object.fromEntries(['images','material','matching','caseGroup'].filter(key=>question[key]!=null||question.metadata?.[key]!=null).map(key=>[key,question[key]??question.metadata[key]])),
  ...
  function paperAccepts(type,question){return TeacherDomains.PaperManagement.PaperQuestionPicker.accepts(type,question);}
  function normalizePaper(paper, index=0){
    ...
    const quotaEnvelope=paper.quotas&&typeof paper.quotas==='object'&&!Array.isArray(paper.quotas)?paper.quotas:{};
    const legacyDomainQuotas=paper.domainTargets&&typeof paper.domainTargets==='object'&&!Array.isArray(paper.domainTargets)
      ?paper.domainTargets
      :(quotaEnvelope.domainQuotas&&typeof quotaEnvelope.domainQuotas==='object'?quotaEnvelope.domainQuotas:quotaEnvelope);
    ...
    const principleQuotas=normalizePaperQuotaMap(paper.principleQuotas||quotaEnvelope.principleQuotas);
    ...
    return {
      ...
      subject,
      paperType:['standard','multiple_choice','mixed'].includes(paper.paperType||paper.paper_type)?(paper.paperType||paper.paper_type):'standard',
      ...
      createdAt:paperTimestamp(paper.createdAt)||Date.now(),
      updatedAt:paperTimestamp(paper.updatedAt)||Date.now(),
      publishedAt:paperTimestamp(paper.publishedAt),
      publishedVersion,
      publishedReleaseId:String(paper.publishedReleaseId || ''),
      withdrawnAt:paperTimestamp(paper.withdrawnAt),
      restoredAt:paperTimestamp(paper.restoredAt),
      ...
      archivedAt:paperTimestamp(paper.archivedAt),
      supplementMode:(paper.supplementMode||quotaEnvelope.supplementMode)==='principle'?'principle':'domain',
      domainQuotas,
      principleQuotas,
      manualQuestionIds,
      quotas:{...domainQuotas},
  ...
  function normalizePaperCategory(category,index=0){
    ...
    return {id:String(category.id||('paper-category-'+index)),name:String(category.name||`分类 ${index+1}`).trim()||`分类 ${index+1}`,description:String(category.description||''),orderIndex:Math.max(0,Number(category.orderIndex||0)),revision:Math.max(1,Number(category.revision||1)),createdAt:paperTimestamp(category.createdAt)||Date.now(),updatedAt:paperTimestamp(category.updatedAt)||Date.now()};
  }
  function paperMetadataPayload(paper){
    return {
      revision:paper.revision,name:paper.name,subject:paper.subject,paperType:paper.paperType,description:paper.description,
      categoryId:paper.categoryId||null,totalCount:Math.max(Number(paper.totalCount||0),paper.questions?.length||0),
      quotas:{supplementMode:paper.supplementMode==='principle'?'principle':'domain',domainQuotas:clone(paper.domainQuotas||{}),principleQuotas:clone(paper.principleQuotas||{})},
      accessPolicy:clone(paper.accessPolicy||{}),enabledModes:clone(paper.enabledModes||[]),modeConfigVersion:Number(paper.modeConfigVersion||PAPER_MODE_CONFIG_VERSION),purpose:String(paper.purpose||'learning')
    };
  }
  function paperCreatePayload(paper){
  ...
  function supplementPaperDraft(value,candidateRows=paperCandidates(value?.subject),random=Math.random){
    ...
    const wantedType=paper.paperType||'standard';
    const rows=(Array.isArray(candidateRows)?candidateRows:[]).filter(row=>paperAccepts(wantedType,row?.question)),candidateById=new Map();
  ...
  function paperDomainStats(subject,candidateRows=paperCandidates(subject)){
    const stats = new Map();
  ...
  function paperPrincipleStats(subject,candidateRows=paperCandidates(subject)){
    ...
    const storedItems=Array.isArray(runtimeItems)?runtimeItems:[];
  ...
  function setCurrentPaper(paper){return paper||null}

  function paperRefKey(ref){return String(ref?.bankId||'')+'::'+String(ref?.questionId||'')}
  // 发布/撤回以 /api/v1/paper-releases 为唯一权威源；草稿状态由 PaperDraftApi 重新加载。
  async function publishPaperRelease(paper){
    if(!window.KGPaperReleaseApi?.publish){toast('试卷发布 API 未加载，请刷新页面后重试。');return null}
    if(!(paper?.questions||[]).length){toast('试卷至少需要一道题目。');return null}
    if(!(paper?.enabledModes||[]).length){toast('请至少选择一种学习模式后再发布。');return null}
    try{
      const published=await window.KGPaperReleaseApi.publish(paper.id,{
  ...
  async function withdrawPaperRelease(paper){
    ...
    try{
      await window.KGPaperReleaseApi.withdrawPaper(paper?.id);
      window.dispatchEvent?.(new CustomEvent('kg:paper-release-published'));
    }catch(error){
  ...
  function renderPaperImportState(snapshot){
    ...
    if(results){
      if(snapshot.busy)results.innerHTML='<div class="qb-empty">正在检查试卷包与题库引用…</div>';
      else if(snapshot.error)results.innerHTML=`<ul class="pm-result-list"><li class="error">${escapeHTML(snapshot.error)}</li></ul>`;
      else if(snapshot.success)results.innerHTML=`<ul class="pm-result-list"><li class="success">导入成功：${escapeHTML(snapshot.success?.paper?.name||snapshot.success?.paper?.id||'试卷草稿')}</li></ul>`;
      else if(preflight){const summary=preflight.summary||{},conflict=preflight.paperConflict;results.innerHTML=`<div class="pm-result-summary"><span class="qb-badge">${escapeHTML(summary.name||'未命名试卷')}</span><span class="qb-badge">${Number(summary.questionCount||0)} 题</span><span class="qb-badge">${Number(summary.sourceBankCount||0)} 个来源题库</span><span class="qb-badge ${preflight.valid?'current':'warn'}">${preflight.valid?'预检通过':'预检未通过'}</span>${conflict?`<span class="qb-badge warn">已存在同 ID ${escapeHTML(conflict.status||'')}</span>`:''}</div><ul class="pm-result-list">${findingRows(preflight.errors,'error')}${findingRows(preflight.warnings,'warning')}${preflight.valid&&!preflight.warnings?.length?'<li class="success">题目引用已全部解析，可确认导入。</li>':''}</ul>`}
  ...
  function syncPaperCompositionForm(){
    if(!paperCompositionController)return;
    paperCompositionController.setPaperType($('qbPaperCompositionType')?.value||'standard');
    paperCompositionController.setBankIds(Array.from(document.querySelectorAll('#qbPaperCompositionBanks [data-paper-composition-bank]:checked')).map(input=>String(input.value||'')));
    for(const code of ['A','B','C'])paperCompositionController.setVariant(code,{enabled:!!$(`qbPaperVariant${code}Enabled`)?.checked,name:$(`qbPaperVariant${code}Name`)?.value||`${code} 卷`,totalCount:Math.max(1,Number($(`qbPaperVariant${code}Count`)?.value||60))});
    paperCompositionController.setHardWeights({people:Math.max(0,Number($('qbPaperPeopleWeight')?.value||0)),process:Math.max(0,Number($('qbPaperProcessWeight')?.value||0)),'business-environment':Math.max(0,Number($('qbPaperBusinessWeight')?.value||0))});
    paperCompositionController.setSoftWeights({governance:Math.max(0,Number($('qbPaperGovernanceWeight')?.value||0)),scope:Math.max(0,Number($('qbPaperScopeWeight')?.value||0)),schedule:Math.max(0,Number($('qbPaperScheduleWeight')?.value||0)),finance:Math.max(0,Number($('qbPaperFinanceWeight')?.value||0)),stakeholder:Math.max(0,Number($('qbPaperStakeholderWeight')?.value||0)),resource:Math.max(0,Number($('qbPaperResourceWeight')?.value||0)),risk:Math.max(0,Number($('qbPaperRiskWeight')?.value||0))});
  }
  ...
  function quotaSummary(values={}){return Object.entries(values).map(([key,value])=>`${key} ${value}`).join('、')||'无'}
  function renderPaperCompositionState(snapshot){
    const results=$('qbPaperCompositionResults'),preflight=snapshot.preflight;
    if(results){
      if(snapshot.busy)results.innerHTML='<div class="qb-empty">正在按硬配额分配题目并检查 A/B/C 卷间重复…</div>';
      else if(snapshot.error)results.innerHTML=`<ul class="pm-result-list"><li class="error">${escapeHTML(snapshot.error)}</li></ul>`;
      else if(snapshot.success)results.innerHTML=`<ul class="pm-result-list"><li class="success">已在同一事务中生成 ${Number(snapshot.success?.papers?.length||0)} 套试卷，批次 ${escapeHTML(snapshot.success?.batchId||'')}。</li></ul>`;
      else if(preflight){results.innerHTML=`<div class="pm-result-summary"><span class="qb-badge">候选 ${Number(preflight.candidateCount||0)} 题</span><span class="qb-badge ${preflight.unclassifiedCount?'warn':''}">未分类 ${Number(preflight.unclassifiedCount||0)} 题</span><span class="qb-badge ${preflight.feasible?'current':'warn'}">${preflight.feasible?'全部可行':'存在库存缺口'}</span><span class="qb-badge">种子 ${escapeHTML(preflight.normalizedRequest?.randomSeed||'')}</span></div>${(preflight.variants||[]).map(item=>`<div class="pm-variant-result"><b>${escapeHTML(item.code||'')}</b><strong class="${item.feasible?'ok':'fail'}">${item.feasible?'可生成':'不可生成'}</strong><span>题量 ${Number(item.totalCount||item.questionIds?.length||0)}；硬配额 ${escapeHTML(quotaSummary(item.hardTargets))}${item.feasible?'':`；缺口 ${escapeHTML(quotaSummary(item.hardShortages))}`}<br/>绩效域目标 ${escapeHTML(quotaSummary(item.softTargets))}；实际 ${escapeHTML(quotaSummary(item.softActual))}；偏差 ${escapeHTML(quotaSummary(item.softDeviation))}</span></div>`).join('')}`}
      else results.innerHTML='<div class="qb-empty">选择题库并预检后，将显示每套卷的题量、硬配额、软目标偏差和库存缺口。</div>';
    }
    if($('qbPaperCompositionConfirmBtn'))$('qbPaperCompositionConfirmBtn').disabled=!preflight?.feasible||snapshot.busy;if($('qbPaperCompositionPreflightBtn'))$('qbPaperCompositionPreflightBtn').disabled=snapshot.busy;if($('qbPaperCompositionRetryBtn')){$('qbPaperCompositionRetryBtn').hidden=!snapshot.error;$('qbPaperCompositionRetryBtn').disabled=snapshot.busy}if($('qbPaperCompositionFeasibleBtn')){$('qbPaperCompositionFeasibleBtn').hidden=!preflight||preflight.feasible||!(preflight.feasibleVariantCodes||[]).length;$('qbPaperCompositionFeasibleBtn').disabled=snapshot.busy}
  }
  function initPaperCompositionControls(){
    const factory=TeacherDomains.PaperManagement?.PaperCompositionController;if(!factory?.create)return;
    paperCompositionController=factory.create({api:PaperDraftApi,onChange:renderPaperCompositionState,onReload:result=>reloadPaperDrafts({selectedId:result?.papers?.[0]?.id})});renderPaperCompositionState(paperCompositionController.snapshot());
    $('qbComposePapersBtn')?.addEventListener('click',()=>{paperCompositionController.cancel();document.querySelector('input[name="paperCompositionMode"][value="quick"]')?.click();applyCompositionMode('quick');syncPaperCompositionForm();openPaperOperationDialog($('qbPaperCompositionDialog'))});
    document.querySelectorAll('input[name="paperCompositionMode"]').forEach(input=>input.addEventListener('change',event=>{if(event.currentTarget.checked)applyCompositionMode(event.currentTarget.value)}));
    $('qbPaperCompositionPreflightBtn')?.addEventListener('click',async()=>{syncPaperCompositionForm();await paperCompositionController.preflight()});
    $('qbPaperCompositionRetryBtn')?.addEventListener('click',async()=>{syncPaperCompositionForm();await paperCompositionController.retry()});
    $('qbPaperCompositionFeasibleBtn')?.addEventListener('click',()=>paperCompositionController.repreflightFeasible());
    $('qbPaperCompositionConfirmBtn')?.addEventListener('click',async()=>{const result=await paperCompositionController.confirm();if(result.ok){toast(`已生成 ${Number(result.result?.papers?.length||0)} 套不重复平行卷。`);closePaperOperationDialog($('qbPaperCompositionDialog'))}});
    $('qbPaperCompositionCancelBtn')?.addEventListener('click',()=>{paperCompositionController.cancel();closePaperOperationDialog($('qbPaperCompositionDialog'))});
  }
  ...
  function initStaticControls(){
    ...
    $('questionTypeInput')?.addEventListener('change', event => {
      const question=currentQuestion();if(!question)return;
      const previous=question.type;
      question.type=String(event.currentTarget.value||'single_choice');
      if(question.type==='multiple_choice'&&previous!=='multiple_choice')question.correctOptionIds=question.correctAnswer?[question.correctAnswer]:[];
      if(question.type!=='multiple_choice'&&previous==='multiple_choice')question.correctAnswer=question.correctOptionIds?.[0]||question.options?.[0]?.id||'';
      renderOptions();window.KGQuestionRichEditor?.setType(question.type);
    });
    ...
    $('paperTypeInput')?.addEventListener('change', async () => {if(await savePaperForm({silent:true,skipRender:true})){state.paperCandidatePage=1;state.selectedPaperCandidateKeys=new Set();await loadPaperCandidatePage({forceReload:true}).catch(()=>{});renderPaperManager()}});
    document.querySelectorAll('[data-paper-mode]').forEach(input=>input.addEventListener('change',()=>{if(currentPaper())savePaperForm({silent:true})}));
    document.querySelectorAll('[data-paper-supplement-mode]').forEach(input=>input.addEventListener('change',handlePaperSupplementModeChange));
  ...
  function questionTypeLabel(type){
    return ({
      single_choice:'单选题',
      multiple_choice:'多选题',matching:'匹配题',
      scenario:'情景题',
      case_analysis:'案例分析题'
    })[type] || type || '未设置题型';
  }
  ...
  function fillQuestionForm(){
    ...
    let richHost=$('questionRichEditor');if(!richHost&&$('qbOptionsEditor')){richHost=document.createElement('div');richHost.id='questionRichEditor';$('qbOptionsEditor').before(richHost);}
    if(richHost)window.KGQuestionRichEditor?.mount(richHost,q,{onCreateChildren:createCaseChildren});
    const disabled = !q;
  ...
  function renderOptions(){
    ...
    wrap.hidden=q.type==='matching';if($('qbAddOptionBtn'))$('qbAddOptionBtn').hidden=q.type==='matching';
    if(q.type==='matching'){wrap.innerHTML='';return;}
    const multiple=q.type==='multiple_choice';
    const correctIds=new Set(multiple?(q.correctOptionIds||[]):[q.correctAnswer]);
    wrap.innerHTML = (q.options || []).map((o, i) => `
      <div class="qb-option-row" data-index="${i}" data-option-id="${escapeHTML(o.id)}">
        <label class="radio">
          <input type="${multiple?'checkbox':'radio'}" name="correctOption" value="${escapeHTML(o.id)}" ${correctIds.has(o.id) ? 'checked' : ''} />
          正确
        </label>
    ...
    wrap.querySelectorAll('.option-remove').forEach(btn => {
      btn.addEventListener('click', e => {
        ...
        if(multiple)q.correctOptionIds=(q.correctOptionIds||[]).filter(id=>q.options.some(option=>option.id===id));
        else if(!q.options.some(o => o.id === q.correctAnswer)) q.correctAnswer = q.options[0]?.id || '';
  ...
  function persistLibraryWorkspaceLayout(){
    writePreference(QUESTION_LIBRARY_WORKSPACE_LAYOUT_KEY,{ratio:state.libraryPaneRatio});
  }
  ...
  function initLibraryWorkspaceControls(){
    try{const saved=readPreference(QUESTION_LIBRARY_WORKSPACE_LAYOUT_KEY,{});if(Number(saved.ratio)>0)state.libraryPaneRatio=Math.min(.75,Math.max(.25,Number(saved.ratio)))}catch(error){}
  ...
  function openLibraryQuestionPreview(questionId,anchor,options={}){
    ...
    const mixedMarkup=window.KGQuestionMaterials?.render(question,{selectedPairs:question.matching?.correctPairs,readOnly:true,reveal:true})||'';$('qbLibraryQuestionPreviewContent').insertAdjacentHTML('afterbegin',mixedMarkup);window.KGQuestionMaterials?.bindMedia($('qbLibraryQuestionPreviewContent'));
  ...
  function persistPaperWorkspaceLayout(){
    writePreference(PAPER_WORKSPACE_LAYOUT_KEY,{ratio:state.paperPaneRatio});
  }
  ...
  function initPaperWorkspaceControls(){
    try{
      const saved=readPreference(PAPER_WORKSPACE_LAYOUT_KEY,{});
  ...
  function questionTypeLabel(type){
    return ({single_choice:'单选题',multiple_choice:'多选题',matching:'匹配题',true_false:'判断题',short_answer:'简答题'}[String(type||'')]||String(type||'题目'));
  }
  ...
  async function openPaperQuestionPreview(value,anchor,options={}){
    ...
    $('qbQuestionPreviewContent').insertAdjacentHTML('afterbegin',window.KGQuestionMaterials?.render(question,{selectedPairs:question.matching?.correctPairs,readOnly:true,reveal:true})||'');window.KGQuestionMaterials?.bindMedia($('qbQuestionPreviewContent'));
  ...
  async function renamePaperCategory(categoryId){
    const category=state.paperCategories.find(item=>item.id===categoryId);if(!category)return;const name=String(prompt('修改分类名称：',category.name)||'').trim();if(!name||name===category.name)return;
    try{const saved=normalizePaperCategory(await PaperDraftApi.updateCategory(categoryId,{revision:category.revision,name}));state.paperCategories=state.paperCategories.map(item=>item.id===categoryId?saved:item);renderPaperManager();toast('分类名称已更新。')}catch(error){if(error?.status===409)await reloadPaperDrafts().catch(()=>{});toast(paperApiMessage(error,'修改分类失败。'))}
  }
  async function deletePaperCategory(categoryId){
    const category=state.paperCategories.find(item=>item.id===categoryId);if(!category)return;const count=state.papers.filter(paper=>paper.categoryId===categoryId).length;
    if(count)return toast(`该分类仍有 ${count} 张试卷，请先批量移到其他分类或“未分类”。`);
    if(!confirm(`确定删除空分类“${category.name}”吗？`))return;
    try{await PaperDraftApi.removeCategory(categoryId,category.revision);state.paperCategories=state.paperCategories.filter(item=>item.id!==categoryId);if(state.paperCategoryFilter===categoryId)state.paperCategoryFilter='UNCATEGORIZED';renderPaperManager();toast('分类已删除。')}catch(error){if(error?.status===409)await reloadPaperDrafts().catch(()=>{});toast(paperApiMessage(error,'删除分类失败。'))}
  }
...
已发布试卷将从学员端下架，历史发布记录仍保留。`))return;
    const failed=[];for(const paper of selected){try{if(isPaperPublished(paper)&&!(await withdrawPaperRelease(paper))){failed.push(paper.name);continue}const latest=await PaperDraftApi.detail(paper.id);await PaperDraftApi.archive(paper.id,latest?.revision||paper.revision)}catch(error){failed.push(paper.name)}}
...
只删除试卷配置，不会删除题库原题。`))return;let removed=0;for(const paper of deletable){try{await PaperDraftApi.remove(paper.id,{revision:paper.revision,reason:'bulk_delete_draft'});removed+=1}catch(error){console.warn('删除试卷草稿失败',paper.id,error)}}state.selectedPaperIds=new Set();await reloadPaperDrafts().catch(()=>{});ensureSelectedPaperVisible();renderPaperManager();toast(`已删除 ${removed} 张草稿${removed<deletable.length?`，${deletable.length-removed} 张失败`:''}${protectedCount?`，跳过 ${protectedCount} 张受保护试卷`:''}。`)
  ...
  async function handlePaperSupplementModeChange(){
    const paper=currentPaper();if(!paper){syncPaperSupplementModeControls(null);return false}
    if(await savePaperForm({silent:true}))return true;
  ...
  function fillPaperForm(){
    ...
    const typeInput=$('paperTypeInput');if(typeInput){typeInput.value=paper?.paperType||'standard';typeInput.disabled=!!paper&&((paper.questions||[]).length>0||Number(paper.publishedVersion||0)>0);typeInput.title=typeInput.disabled?'加入题目或发布后不能更改试卷类型。':''}
  ...
  function readPaperFormInto(paper){
    ...
    paper.paperType = $('paperTypeInput')?.value||'mixed';
  ...
  async function removeSelectedPaperQuestions(){
...
只移除试卷引用，不会删除题库原题。`))return;
    const draft=clone(paper),response=teacherDomainServices().paperPicker?.remove?.(draft,[...keys]);if(!response?.ok)return toast(response?.errors?.[0]||'移除题目失败。');const remainingIds=new Set((draft.questions||[]).map(ref=>String(ref.questionId||'')));draft.manualQuestionIds=(draft.manualQuestionIds||[]).filter(id=>remainingIds.has(String(id)));const saved=await persistPaperQuestions(draft,{silent:true});if(!saved)return;state.paperQuotaFeedback=null;state.selectedPaperQuestionKeys=new Set();renderPaperManager();toast(`已从试卷移除 ${response.value.removed} 道题。`);return response;
  }
  async function movePaperQuestion(index,direction){const paper=currentPaper(),target=index+direction;if(!paper||index<0||target<0||target>=paper.questions.length)return;const draft=clone(paper);const moved=teacherDomainServices().paperPicker.moveGroup(draft,paperRefKey(draft.questions[index]),direction);if(!moved.ok)return;if(await persistPaperQuestions(draft,{silent:true}))renderPaperQuestionList()}
  ...
  async function loadPaperCandidatePage(options={}){
    ...
    return paperDataLoader.selectBank(state.paperCandidateBankId,{
      page:state.paperCandidatePage,pageSize:PAPER_CANDIDATE_PAGE_SIZE,search:state.paperCandidateSearch,questionType:paper?.paperType||'standard',forceReload:options.forceReload===true
    });
  }
  function renderPaperCandidateList(){
    ...
    const wantedType=paper.paperType||'standard',rows=state.paperCandidateRows.filter(row=>row.bank?.id===state.paperCandidateBankId&&!isQuestionDeleted(row.question)).filter(row=>paperAccepts(wantedType,row.question)),existing=new Set((paper.questions||[]).map(paperRefKey));
  ...
  async function addSelectedCandidatesToPaper(){
    const paper=currentPaper();if(!paper)return toast('请先新建试卷。');const selected=state.selectedPaperCandidateKeys;if(!selected.size)return toast('请先选择当前页题目。');await loadPaperQuotaCandidates(paper.subject);const refs=[...selected].map(key=>{const [bankId,questionId]=key.split('::');return {bankId,questionId,score:1}}).filter(ref=>ref.bankId&&ref.questionId),draft=clone(paper);const response=teacherDomainServices().paperPicker?.add?.(draft,refs);if(!response?.ok)return toast(response?.errors?.[0]||'加入题目失败。');draft.manualQuestionIds=[...new Set([...(draft.manualQuestionIds||[]),...response.value.added.map(ref=>String(ref.questionId||'')).filter(Boolean)])];draft.totalCount=Math.max(Number(draft.totalCount||0),draft.questions.length);const saved=await persistPaperQuestions(draft,{silent:true});if(!saved)return;state.paperQuotaFeedback=null;state.selectedPaperCandidateKeys=new Set();renderPaperManager();toast(`已加入 ${response.value.added.length} 道题${response.value.duplicates.length?`，跳过 ${response.value.duplicates.length} 道重复题`:''}。`);return response;
  }
  ...
  async function autoDistributeQuota(){
    ...
    const candidates=paperCandidates(paper.subject).filter(row=>paperAccepts(wantedType,row.question));
    const stats=(mode==='principle'?paperPrincipleStats(paper.subject,candidates).map(item=>({bucketId:item.id,count:item.count})):paperDomainStats(paper.subject,candidates).map(item=>({bucketId:item.domain,count:item.count}))).filter(item => item.count > 0);
  ...
  async function togglePublishPaper(){
    ...
    if(!(await savePaperForm({silent:true,skipRender:true})))return;paper=currentPaper();
...
归档后学员端不再显示，历史发布记录仍保留。`))return;
    ...
    try{const latest=await PaperDraftApi.detail(paper.id);await PaperDraftApi.archive(paper.id,latest?.revision||paper.revision);setCurrentPaper(null);await reloadPaperDrafts({selectedId:paper.id});renderPaperManager();toast('试卷已归档。')}catch(error){toast(paperApiMessage(error,'归档试卷失败。'))}
  }
  async function unarchiveCurrentPaper(){
...
试卷将恢复为可编辑草稿，不会自动重新发布。`))return;
    try{await PaperDraftApi.restore(paper.id,paper.revision);await reloadPaperDrafts({selectedId:paper.id});renderPaperManager();toast('已取消归档，可继续编辑并发布新版本。')}catch(error){toast(paperApiMessage(error,'取消归档失败。'))}
  }
  async function removePaperQuestion(index){
    const paper = clone(currentPaper());
    if(!paper) return;
    const removed=paper.questions[index];
    teacherDomainServices().paperPicker.remove(paper,[paperRefKey(removed)]);
    const remainingIds=new Set(paper.questions.map(ref=>String(ref.questionId)));paper.manualQuestionIds=(paper.manualQuestionIds||[]).filter(id=>remainingIds.has(String(id)));
  ...
  async function deleteCurrentPaper(){
    ...
    try{await PaperDraftApi.remove(paper.id,{revision:paper.revision,reason:'delete_draft'});setCurrentPaper(null);await reloadPaperDrafts();renderPaperManager();toast('已删除试卷。')}catch(error){toast(paperApiMessage(error,'删除试卷失败。'))}
  }
  async function exportCurrentPaper(){
    if(!(await savePaperForm({silent:true, skipRender:true})))return;
    const paper = currentPaper();
    if(!paper) return;
  ...
  function collectOptionsFromDom(){
    ...
    const correct = new Set(Array.from(document.querySelectorAll('input[name="correctOption"]:checked')).map(input=>String(input.value||'')));
  ...
  function collectQuestionDraftFromDom(q=currentQuestion(),bank=currentBank(),options={}){
    ...
    const selected=draft.options.filter(option=>option.correct).map(option=>option.id);
    if(draft.type==='multiple_choice'){
      draft.correctOptionIds=AnswerSet.normalizeIds?.(selected,draft.options.map(option=>option.id))||selected;
      draft.correctAnswer='';
      draft.options.forEach(option=>{option.correct=draft.correctOptionIds.includes(option.id)});
    }else{
      const correct=selected[0]||draft.options[0]?.id||'';
      draft.correctAnswer=correct;
      draft.correctOptionIds=[];
      draft.options.forEach(option=>{option.correct=option.id===correct});
    }
    ...
    return {draft:window.KGQuestionRichEditor?.collect(draft)||draft,rawStem,reasoningSteps};
  }
  async function saveQuestionForm(options={}){
    ...
    try{await window.KGQuestionRichEditor?.flush(q);}catch(error){if(!options.silent)alert(error.message);return false;}
    const collected=collectQuestionDraftFromDom(q,bank);
    if(!collected)return false;
    const {draft,rawStem,reasoningSteps}=collected;
    const answerErrors=AnswerSet.validate?.(draft)||[];
    if(answerErrors.length){if(!options.silent)alert(answerErrors.join('\n'));return false}
    ...
    const hasAnswer=draft.type==='matching'?!!AnswerSet.pairs(draft,draft.matching?.correctPairs,true):draft.type==='multiple_choice'?draft.correctOptionIds.length>=2:!!draft.correctAnswer;
    draft.status={...(draft.status||{}),contentReady:!!(rawStem.trim()&&(draft.type==='matching'||draft.options.length>=2)&&hasAnswer),published};
  ...
  async function createCaseChildren(){
    if(!await saveQuestionForm({silent:true}))throw new Error('请先补全并保存当前小题。');
    const bank=currentBank(),q=currentQuestion(),group=q.caseGroup;if(!group)throw new Error('请先关联案例材料。');
    const existing=new Set(bank.questions.filter(item=>item.caseGroup?.id===group.id).map(item=>item.caseGroup.order));
    try{for(let order=1;order<=group.total;order++){if(existing.has(order))continue;const child=emptyQuestion(bank.subject);child.title=q.material.title+' · 小题 '+order;child.material=clone(q.material);child.caseGroup={...group,order};child.status={...child.status,contentReady:false};await Catalog.saveQuestion(child,{bankId:bank.id});}
      reloadBanksFromCatalog(bank.id,q.id);render();toast('案例小题已补齐，请逐题填写题干、答案和解析。');
    }catch(error){reloadBanksFromCatalog(bank.id,q.id);throw error;}
  }
  ...
  async function clearCurrentBankTestRecords(){
    ...
    if(!confirmed)return;
    try{
      if(!Catalog?.clearBankTestRecords)throw new Error('题库 API 未加载。');
      const payload=await Catalog.clearBankTestRecords(bank.id);
...
此操作不可恢复。`;
  ...
  async function previewDeepRecall(){
    ...
    try{
      ...
      const payload={question:payloadQuestion,savedAt:Date.now(),source:'question-bank-admin-preview',previewMode:'teacher-draft',previewToken,sourceBankId:bank?.id||'',sourceQuestionId:q.id||'',userId:window.KGAuthCore?.currentUsername?.()||'guest'};
      if(!window.KGRecallStorage?.writeCurrent)throw new Error('回忆预览 API 未加载');
      const saved=window.KGRecallStorage.writeCurrent(payload);
  ...
  function downloadTemplate(){
    const template = normalizeBank({
      ...
      version:'1.0',
      visibility:'template',
      questions:[demoQuestion(),multipleChoiceTemplateQuestion()]
    });
  ...
  async function importQuestionBanks(data){
    ...
    try{
      ...
      const embeddedPaperCount=Array.isArray(data?.papers)?data.papers.length:0;
      ...
      toast(`已导入并保存 ${savedBanks.length} 个题库${skipped?`，跳过 ${skipped} 个相同题库`:''}${embeddedPaperCount?`；检测到 ${embeddedPaperCount} 套试卷，请在“试卷管理”使用 JSON 导入`:''}。`);
      return {ok:true,bankId:state.selectedBankId,questionId:state.selectedQuestionId,importedBankCount:savedBanks.length,importedPaperCount:0};
    }catch(error){
      restoreQuestionImportState(snapshot);render();
  ...
  function showApiStartupError(message){
    const host=document.querySelector('.qb-app')||document.body;
    let panel=$('qbApiStartupError');
    if(!panel){panel=document.createElement('section');panel.id='qbApiStartupError';panel.className='qb-empty';panel.setAttribute('role','alert');host.prepend(panel)}
    panel.innerHTML=`<strong>${escapeHTML(message)}</strong><p>可以检查网络或重新登录后再试，当前页面不会使用本地数据回退。</p><button type="button" data-api-retry>重新加载</button>`;
    panel.querySelector('[data-api-retry]')?.addEventListener('click',()=>window.location.reload());
    return panel;
  }
  ...
  function canonicalQuestionDuplicateSignature(question={}){
    ...
    return JSON.stringify({stem:canonicalDuplicateText(primaryStem),options,correctAnswer:canonicalDuplicateText(question.correctAnswer),...AnswerSet.contentExtension(question)});
  }
