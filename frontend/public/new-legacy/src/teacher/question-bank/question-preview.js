...
(function(global){
  ...
  function render(question,options={}){const model=viewModel(question,options.language||'zh');return `<article class="teacher-question-preview"><h3>${escape(model.title)}</h3><p class="stem">${escape(model.stem)}</p>${global.KGQuestionMaterials?.render(question,{readOnly:true,reveal:true,selectedPairs:question.matching?.correctPairs})||''}<div class="options">${model.options.map(item=>`<div class="option ${item.correct?'correct':''}"><b>${escape(item.id)}</b><span>${escape(item.text||'—')}</span>${item.correct?'<em>正确答案</em>':''}</div>`).join('')}</div><section class="analysis"><b>解析</b><p>${escape(model.analysis||'暂无解析')}</p></section></article>`}
