...
(function(global){
  ...
  class TaxonomyService{
    ...
    async importVersion(payload,options={}){
      ...
      const all=this.list(),tx=await this.transactions.executeAsync({name:'导入知识树版本',action:'taxonomy.import',entityType:'taxonomy',entityId:id,permission:'importTaxonomies',keys:['taxonomies','taxonomyImports'],validate:()=>validation,commit:async()=>{const saved=await this.legacy.saveTaxonomies([...all,imported]);if(!saved.valid)return saved;const records=this.repository.read('taxonomyImports',[]),record={id:Core.safeId('taxonomy-import'),importedAt:now,importedBy:actor,fileName:Core.clean(options.fileName),subjectId:parsed.subjectId,taxonomyId:id,version,versionLabel:this.versionLabel(version),nodeCount:imported.nodes.length,result:'success'};this.repository.write('taxonomyImports',[record,...(Array.isArray(records)?records:[])].slice(0,100));return {valid:true,taxonomy:this.get(id),importRecord:record}},metadata:{subjectId:parsed.subjectId,version,nodeCount:imported.nodes.length,fileName:Core.clean(options.fileName)}});
