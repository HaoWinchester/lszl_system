...
/* 登录态判定：进入本页不要求登录。同步使用 auth-core/direct bootstrap，
   再异步调 /api/v1/auth/me 用服务端 cookie 校验补齐。 */
function readCachedSessionUser(){
  try{
    const user=window.KGAuthCore?.currentUser?.({includeInactive:true})||window.__KG_DIRECT_BOOTSTRAP__?.authUser||null;
