...
with sync_playwright() as p:
    ...
    # Failed or pending insertion must not look like a successful close.
    page.evaluate("""()=>{KGMultiQuestionWorkspace.addQuestionByReference=()=>Promise.resolve({created:false,message:'该试卷已撤回，不能放入画布。'})}""")
    page.locator('[data-mistake-action="insert"]').click()
    page.wait_for_timeout(20)
    assert page.locator('#qwMistakesDrawer').is_visible()
    assert '该试卷已撤回' in page.locator('#qwMistakesError').inner_text()
    page.evaluate("""()=>{KGMultiQuestionWorkspace.addQuestionByReference=()=>new Promise((resolve,reject)=>{window._resolveInsert=resolve;window._rejectInsert=reject})}""")
    page.locator('[data-mistake-action="insert"]').click()
    assert page.locator('#qwMistakesDrawer').is_visible()
    assert page.locator('[data-mistake-action="insert"]').is_disabled()
    page.evaluate("window._rejectInsert(new Error('网络暂时不可用，请重试'))")
    page.wait_for_function("!document.querySelector('[data-mistake-action=insert]').disabled")
    assert page.locator('#qwMistakesDrawer').is_visible()
    assert '网络暂时不可用' in page.locator('#qwMistakesError').inner_text()
    page.locator('[data-mistake-action="insert"]').click()
    page.evaluate("window._resolveInsert({created:true})")
    page.wait_for_function("document.querySelector('#qwMistakesDrawer').hidden")
    page.locator('#qwMistakesBtn').click()
