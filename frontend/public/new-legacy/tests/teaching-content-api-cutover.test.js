...
test('historical taxonomy cycles cannot trap descendant traversal', () => {
  const source = readSource('src/91-learning-content-core.js');
  const body = source.match(/function descendantIds[\s\S]*?\n  }/)?.[0] || '';
  assert.match(body, /visited=new Set/);
  assert.match(body, /visited\.has\(id\)/);
});
