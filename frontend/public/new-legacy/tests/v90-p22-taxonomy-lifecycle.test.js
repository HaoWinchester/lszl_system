...
const teacherDraft=service.importVersion(payload,{fileName:'teacher-draft.json'});
assert(teacherDraft.valid,JSON.stringify(teacherDraft));
const denied=service.deleteVersion(teacherDraft.taxonomy.id);
assert(!denied.valid,'teacher must not delete taxonomy versions');
assert((denied.errors||[]).some(message=>message.includes('权限')));

role='admin';
const removedDraft=service.deleteVersion(teacherDraft.taxonomy.id);
...
const v2=service.importVersion(payload,{fileName:'v2.json'});
assert(v2.valid,JSON.stringify(v2));
assert(service.publish(v2.taxonomy.id,{notes:'publish v2'}).valid);
const v3=service.createDraftFrom(v2.taxonomy.id);
assert(v3.valid,JSON.stringify(v3));
assert(service.publish(v3.taxonomy.id,{notes:'publish v3'}).valid);
assert.equal(service.currentForSubject('subject-pmp').id,v3.taxonomy.id);

const currentDelete=service.deleteVersion(v3.taxonomy.id);
...
const directPublishedDelete=service.deleteVersion(v2.taxonomy.id);
...
const archived=service.archive(v2.taxonomy.id,{notes:'old version'});
assert(archived.valid,JSON.stringify(archived));
assert.equal(service.get(v2.taxonomy.id).status,'archived');
const restored=service.restoreArchived(v2.taxonomy.id);
...
assert(service.archive(v2.taxonomy.id).valid);

const activity=W.KGLearningContent.getActivities()[0];
const mapped=W.KGLearningContent.mapActivities([activity.id],{taxonomyId:v2.taxonomy.id,primaryNodeId:'life-root',relatedNodeIds:[]});
assert(mapped.valid,JSON.stringify(mapped));
references.invalidate();
const blocked=service.deleteVersion(v2.taxonomy.id);
...
const remapped=W.KGLearningContent.mapActivities([activity.id],{taxonomyId:v3.taxonomy.id,primaryNodeId:'life-root',relatedNodeIds:[]});
assert(remapped.valid,JSON.stringify(remapped));
references.invalidate();
const deleted=service.deleteVersion(v2.taxonomy.id,{notes:'no longer used'});
