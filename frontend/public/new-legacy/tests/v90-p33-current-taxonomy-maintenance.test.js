...
const first=await service.saveNode(current.id,{parentId:'kp-pmp-agile',title:{zh:'运营新增节点',en:'Operational Node'},description:{zh:'用于 P3.3 日常维护测试'},code:'PMP.AGILE.OPS',status:'active'});
assert(first.valid,JSON.stringify(first));
assert.equal(first.node.description.zh,'用于 P3.3 日常维护测试');
const firstId=first.node.id;

const edited=await service.saveNode(current.id,{...first.node,title:{zh:'运营节点（已修改）',en:'Operational Node'},description:{zh:'说明已更新'}});
...
const sibling=await service.saveNode(current.id,{parentId:'kp-pmp-agile',title:{zh:'运营同级节点'},status:'active'});
...
const reordered=await service.reorderNode(current.id,sibling.node.id,'up');
...
const moved=await service.saveNode(current.id,{...edited.node,parentId:'kp-pmp-hybrid'});
...
for(let level=7;level<=9;level++){
  const result=await service.saveNode(current.id,{parentId,title:{zh:`第 ${level} 层运营节点`},status:'active'});
  assert(result.valid,JSON.stringify(result));
  assert.equal(result.node.level,level);
  parentId=result.node.id;
}
const tooDeep=await service.saveNode(current.id,{parentId,title:{zh:'第 10 层错误节点'},status:'active'});
assert(!tooDeep.valid,'level 10 must be blocked');
assert((tooDeep.errors||[]).some(message=>message.includes('最多支持 9 层')));
const invalidStatus=await service.saveNode(current.id,{...edited.node,status:'unknown'});
...
const linkedDeprecation=await service.deprecateNode(current.id,'kp-pmp-rtm-bidirectional');
assert(!linkedDeprecation.valid,'node with active children must be handled before deactivation');

for(const nodeId of [parentId]){
  const result=await service.deprecateNode(current.id,nodeId);
  assert(result.valid,JSON.stringify(result));
  const restored=await service.restoreNode(current.id,nodeId);
  assert(restored.valid,JSON.stringify(restored));
  const deleted=await service.deleteNode(current.id,nodeId);
  assert(deleted.valid,JSON.stringify(deleted));
}

const referencedLeaf=await service.saveNode(current.id,{parentId:'kp-pmp-agile',id:'p33-referenced-leaf',title:{zh:'被题目引用的叶节点'},status:'active'});
...
assert((await W.KGLearningContent.saveActivity(library['activity-p33-linked'])).valid);
...
const deactivated=await service.deprecateNode(current.id,'p33-referenced-leaf');
...
const restoredLeaf=await service.restoreNode(current.id,'p33-referenced-leaf');
assert(restoredLeaf.valid);

const emptyLeaf=await service.saveNode(current.id,{parentId:'kp-pmp-hybrid',title:{zh:'可删除空节点'},status:'active'});
assert(emptyLeaf.valid);
const emptyDelete=await service.deleteNode(current.id,emptyLeaf.node.id);
assert(emptyDelete.valid,JSON.stringify(emptyDelete));

const imported=await service.importVersion({id:'taxonomy-pmp-major',subjectId:'subject-pmp',name:{zh:'重大调整树'},nodes:[{id:'major-root',parentId:null,level:1,title:{zh:'重大调整'},status:'active'}]},{fileName:'major.json'});
assert(imported.valid);
const published=await service.publish(imported.taxonomy.id);
assert(published.valid);
const historical=service.get(current.id);
assert.equal(service.editMode(historical),'');
assert(!(await service.saveNode(historical.id,{...historical.nodes[0],title:{zh:'历史版本不可修改'}})).valid,'historical published version must remain read-only');
