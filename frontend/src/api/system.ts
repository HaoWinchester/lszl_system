...
export const systemApi = {
  ...
  wechatPayConfig: () =>
    apiClient.get<{ config: Record<string, unknown> }>('/system/wechat-pay-config').then((r) => r.data.config),
  updateWechatPayConfig: (body: Record<string, unknown>) =>
    apiClient.put<{ config: Record<string, unknown> }>('/system/wechat-pay-config', body).then((r) => r.data.config),
  plans: () =>
