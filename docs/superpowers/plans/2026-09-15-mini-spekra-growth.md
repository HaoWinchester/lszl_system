...
```
- [x] Run .venv/bin/python -m pytest tests/test_practice_growth.py -q and record expected missing-feature failures.
- [x] Implement dedicated owner-keyed settings, day snapshots and daily answer uniqueness with PostgreSQL upsert and appropriate owner/day serialization. Target updates freeze today's old goal before changing future configuration. Derive streak/weekly summaries from persisted daily rows using ZoneInfo('Asia/Shanghai'). Do not trust client timestamps or arbitrary LearningEvent submissions.
```python
...
```
- [x] Create/check migration using project Alembic workflow, run targeted tests, run adjacent practice and experience tests; record results and commit task files only.
...
- [x] Add failing handler tests for growth loading/error/retry, goal saving/failure and next-day notice, milestone/weekly formatting, home resume vs start and growth failures not hiding practice, four-tab navigation and mode propagation. Existing tests must be updated to approved navigation/visual requirements while retaining behavior coverage.
```javascript
...
```
- [x] Establish warm near-white background (#f8f7f2 family), forest main surfaces (#174d3b family), orange primary action with readable dark label, mint secondary emphasis, consistent rounded cards and line icons. Draw simple learning-progress artwork using native layout/CSS or self-authored SVG, not third-party assets. Maintain dark and text-size variants via existing appearance system.
- [x] Implement homepage hierarchy: greeting and goal chip, short learning headline, rich green continue/start card plus complementary wrong-question card, goal progress and real statistics, recent papers. No fake recommended content/counts.
- [x] Promote paper catalog to 练习 tab with subject/search and mode selector; implement 成长 page goal picker, today's progress, seven-day calendar, current/longest streak, milestones and links to history/report flows. Profile compact identity card and grouped functional rows; reading UI large clean question surface and persistent accessible actions.
- [x] Use semantic loading/error/empty states, loading/disabled buttons and real API handlers. New page actions obey existing login/permission/access behavior. Preserve mixed matching/case/image rendering and all practice lifecycle behavior.
- [x] Run node --experimental-strip-types --test miniprogram/tests/*.test.mjs; capture and review rendered screenshots where environment permits; fix all regressions and commit task files only.


Task 1 TDD evidence correction: the initial missing-test-file run was not a meaningful behavioral RED. Subsequent concrete regression RED/GREEN records and independent reviews are archived under artifacts/spekra-growth/review-evidence/.
