...
16. 迁移时若同一个原题库 ID 对应多个 owner，第一份保留原题库 ID，后续题库改为 `-2`、`-3` 后缀；其中名称相同的题库同时依次增加全角序号 `（2）`、`（3）`，题目的 `bank_id` 随映射更新。已经拥有不同 ID 的普通同名题库不属于本次冲突迁移范围。
...
- Prep Studio 新建题目必须使用其生成的 UUID 作为主键，服务器不替换 ID。迁移得到的历史题目 ID 原样保留；只有同名逻辑题库发生 ID 冲突时，才按第 14 节规则重映射题库 ID，题目 ID 不随题库重命名。
...
3. 先按 `(owner, 原题库 ID)` 识别逻辑题库并合并同一 owner 的关系表/Runtime State 副本，再按原题库 ID 查找跨 owner 冲突组；已经拥有不同 ID 的普通同名题库不参与重映射。
4. 冲突组按确定性顺序编号：已存在的关系表题库优先保留原名称和原 ID，其余按 owner 名称、来源和原 ID 排序；第二个起题库 ID 使用 `原ID-2`、`原ID-3`，并同步更新其题目的 `bank_id`。若组内名称去除首尾空白后相同，名称同时使用 `名称（2）`、`名称（3）`；名称原本不同则保持原名称。
5. 新题库 ID 必须全局唯一且不超过 64 字符。若直接后缀已存在或超长，使用保留序号的截断 ID 加原 ID、owner 和序号的短哈希，仍不得覆盖现有题库。
6. 迁移报告必须输出每条 `owner / oldBankId / newBankId / oldName / newName / ordinal` 映射。相同输入重复 dry-run 或 apply 必须得到完全相同的映射。
7. 同题目 ID、同规范 content hash 合并为一条；同题目 ID、不同 hash 不静默覆盖。题库重映射后若暴露题目 ID 冲突，仍阻止正式切换并输出冲突清单，不自动重编号题目。
8. 为已有私有题库设置原 owner，为共享题库保留发布可见性并补齐管理者。
9. 迁移完成后比对题库数、题目数、公开/内部数量和抽样完整字段。
10. 验证通过后切换主项目读取 API，并禁止 Runtime State 正式题库写入。
11. 快照保留到完成至少一次生产回滚演练和验收，不在迁移脚本中删除原数据。

当前真实数据的确定性映射顺序为 `admin`、`佩奇007`、`老师`。例如：

```text
PMP 敏捷场景题示例题库      -> bank-pmp-demo
PMP 敏捷场景题示例题库（2） -> bank-pmp-demo-2
PMP 敏捷场景题示例题库（3） -> bank-pmp-demo-3
```

只修改迁移目标关系表，不回写或删除 Runtime State 快照。当前数据已只读确认三份 PMP 题库的题目 ID 没有交集；通用迁移仍保留题目冲突的 fail-closed 保护。
