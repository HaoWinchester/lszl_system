...
16. 迁移时若多个逻辑题库名称相同，第一个保留原名称和原题库 ID，后续题库依次在名称后增加全角序号 `（2）`、`（3）`，并把题库 ID 同步改为 `-2`、`-3` 后缀；题目的 `bank_id` 随映射更新。
...
3. 先按 `(owner, 原题库 ID)` 识别逻辑题库并合并同一 owner 的关系表/Runtime State 副本，再按去除首尾空白后的精确题库名称分组。
4. 同名逻辑题库按确定性顺序编号：已存在的关系表题库优先保留原名称和原 ID，其余按 owner 名称、来源和原 ID 排序；第二个起使用 `名称（2）`、`名称（3）`，题库 ID 使用 `原ID-2`、`原ID-3`，并同步更新其题目的 `bank_id`。
