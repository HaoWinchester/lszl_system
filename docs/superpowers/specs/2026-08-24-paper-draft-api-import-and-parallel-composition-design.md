...
试卷管理页同时保留两个独立入口，不能用一个入口猜测或混用两类 JSON：

- **导入题库**：接受 Prep Studio 题库 JSON。单题库文件的顶层直接包含 `id`、`name`、`subject`、`version` 和 `questions`；同时兼容已有题库页支持的数组以及 `{banks: [...]}` 包装格式。该入口复用公共题库导入能力和 `POST /api/v1/banks/import`，沿用同来源覆盖确认、重复题清理确认、事务回滚与导入后目录刷新规则。
- **导入试卷**：只接受 `schema: "kg-paper-package-v1"`、`schemaVersion: 1` 且包含 `paper` 的试卷包。该入口继续调用试卷预检和正式导入 API，只创建题目引用，不复制题目正文。

上传到错误入口时，页面必须先识别另一类已知格式并给出明确引导，例如“检测到题库 JSON，请使用‘导入题库’”；不得再向题库文件罗列“缺少 paper 对象”等误导性试卷字段错误。

题库导入不能在试卷页面复制题库页面的 fetch、覆盖确认或重复题处理代码。公共题库导入 controller/service 负责文件解析、输入格式归一化、冲突确认、错误恢复和 API 提交，题库页与试卷页只负责各自的 DOM 渲染和打开/关闭弹窗。
...
- 真实 60 题 Prep Studio 单题库 JSON 可从“导入题库”提交到题库 API；
- 单题库、题库数组和 `{banks: [...]}` 三种题库输入统一归一化；
- 题库 JSON 上传到“导入试卷”时显示入口引导，不显示 paper 字段错误；
- 试卷包上传到“导入题库”时显示入口引导，不提交题库 API；
- 题库覆盖确认、重复题清理确认、取消与重试在题库页和试卷页行为一致；
- 导入预检、名称差异、缺失引用和冲突策略；
...
- 本文成功标准全部有自动化或浏览器证据。
- 60 题 Prep Studio 题库样本能通过“导入题库”写入数据库，刷新后仍可见并可作为组卷候选。
...
- 自动化验证和用户在 `uat` 环境人工验收均通过后，才合入 `main`、推送远端并清理功能分支。
