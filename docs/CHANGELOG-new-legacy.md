...
- **单机版功能回补第 3 轮：小件收尾（2026-08-21，v9.0-p4.1.127）**：新增 `content-prep-studio/src/js/22-round3-extras.js`——①开发变更记录面板（DEVELOPMENT_CHANGELOG 含 V2.1–V2.6 单机版历史 + V2.7-S 服务器回补记录）与 JSON 导出；②审计日志 JSON 导入（auditId 去重、完全重复跳过、同 ID 异内容判冲突保留本机）；③科目 Facet Registry 独立导出按钮；④联想验收页"同步当前工作区 / 清空临时数据源"。**经核实无需单独移植**：旧原则库/归纳卡迁移已由"原则与归纳卡 JSON"安全合并入口承接（支持旧格式），题目列表置顶/收起已在第 2 轮交付，验收页临时载入外部 JSON 本就存在。浏览器 E2E：7 条开发记录、导入去重正确、按钮齐备，0 页面错误。**至此单机版 P4.5.29 与服务器版的功能差异回补全部完成（除计划内裁剪的本地工作区自动保存/恢复）。**
