...
class QuestionComment(Base):
    ...
    parent_id: Mapped[str | None] = mapped_column(String(64), ForeignKey("question_comments.id", ondelete="SET NULL"), nullable=True, index=True)
...
class QuestionCommentFavorite(Base):
    __tablename__ = "question_comment_favorites"
    comment_id: Mapped[str] = mapped_column(String(64), ForeignKey("question_comments.id", ondelete="CASCADE"), primary_key=True)
