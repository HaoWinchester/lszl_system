...
class Settings(BaseSettings):
    ...
    # Content Prep 题目级校验临时关闭（2026-08 录入提速需求,所有环境默认关闭,免逐机配置）：
    # true 时共享草稿同步/批量上传/单题保存跳过题目校验阻断（仅记录 warning 日志，批次内重复 ID 仍拦截）。
    # 恢复校验：把默认值改为 False（或临时设环境变量 CONTENT_PREP_VALIDATION_DISABLED=false 不重启代码）。
    CONTENT_PREP_VALIDATION_DISABLED: bool = True
