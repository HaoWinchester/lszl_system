...
function redirectFrom(search) {
  ...
  const script = [...html.matchAll(/<script>([\s\S]*?)<\/script>/g)]
    .map(match => match[1])
    .find(source => source.includes('location.replace'));
  ...
  vm.runInNewContext(script, {
    ...
    URLSearchParams,
    location: {
      href: `https://study.example/question-training.html${search}`,
...
test('retired single-deep links redirect to practice with only encoded allow-listed parameters', () => {
  ...
  assert.deepEqual([...target.searchParams.entries()], [
    ...
    ['taskId', 'task+6'],
    ['subject', '项目&管理'],
    ['returnTo', '/learning-path.html'],
    ['retiredMode', 'single_deep_study'],
  ]);
...
test('retired single-deep redirect rejects cross-origin return targets and never boots retired scripts', () => {
  for (const value of ['https://evil.example/steal', '//evil.example/steal', 'javascript:alert(1)']) {
    const target = redirectFrom(`?returnTo=${encodeURIComponent(value)}`);
    assert.equal(target.searchParams.has('returnTo'), false, `must reject ${value}`);
  }

  const html = fs.readFileSync(questionTrainingPath, 'utf8');
  const sources = [...html.matchAll(/<script\b[^>]*\bsrc=["']([^"']+)["'][^>]*>/gi)].map(match => match[1]);
  assert.deepEqual(sources, [], 'the redirect shell must not load the retired application');
});
...
test('practice reads the retirement marker once and prioritizes a valid target question', () => {
  ...
  assert.deepEqual(navigation, {
    ...
    notice: '单题深学已停用，已为你切换到刷题',
  });
  assert.equal(practice.readRetiredModeNavigation('?retiredMode=another_mode&questionId=question-2'), null);
  assert.deepEqual(practice.prioritizeRetiredQuestion([
    { id: 'question-1' },
    { id: 'question-2' },
    { id: 'question-3' },
  ], navigation.questionId).map(question => question.id), ['question-2', 'question-1', 'question-3']);
  assert.deepEqual(practice.prioritizeRetiredQuestion([
    { id: 'question-1' },
    { id: 'question-2' },
  ], 'missing').map(question => question.id), ['question-1', 'question-2']);
});
...
test('practice displays the retirement notice once during the redirected navigation', async () => {
  const original = {
    KGQuestionCatalogAdapter: global.KGQuestionCatalogAdapter,
  ...
  const elementFor = id => {
    if (!elements.has(id)) elements.set(id, createPracticeElement());
  ...
  try {
    global.document = {
      ...
      body: createPracticeElement(),
      getElementById: elementFor,
      querySelector() { return createPracticeElement(); },
      querySelectorAll() { return []; },
    };
    global.location = { search: '?retiredMode=single_deep_study&questionId=question-2' };
    global.addEventListener = () => {};
    global.KGQuestionCatalogAdapter = { ready: Promise.resolve() };
    delete require.cache[practicePath];
    ...
    assert.equal(notice.textContent, '单题深学已停用，已为你切换到刷题');
    await listeners.get('DOMContentLoaded')();
    assert.equal(notice.textContent, '单题深学已停用，已为你切换到刷题');
  } finally {
...
test('retired navigation does not restore an older active practice session', async () => {
  const original = {
    KGQuestionCatalogAdapter: global.KGQuestionCatalogAdapter,
    KGPracticeMode: global.KGPracticeMode,
    addEventListener: global.addEventListener,
    document: global.document,
  ...
  const questions = Array.from({ length: 10 }, (_, index) => ({
    id: `question-${index + 1}`,
    stem: `题目 ${index + 1}`,
    options: [{ id: 'A', text: '答案 A', correct: true }, { id: 'B', text: '答案 B' }],
    correctAnswer: 'A',
  }));
  const published = [{
    id: 'release-1', paperId: 'paper-1', version: 1, status: 'published',
    questions: questions.map((question, index) => ({ bankId: 'bank-1', questionId: question.id, order: index })),
    questionSnapshots: questions.map(question => ({ bankId: 'bank-1', questionId: question.id, question })),
  }];
  const staleAttempt = {
    savedAt: Date.now(), paperId: 'paper-1', releaseId: 'release-1', paperVersion: 1,
    selectedCount: 10, order: 'paper', mode: 'challenge', questions, index: 4, health: 3,
    streak: 0, experience: 0, correct: 0, answered: 4, startedAt: Date.now(), deadline: 0,
  };
  ...
  try {
    ...
    global.location = { search: '?retiredMode=single_deep_study&paperId=paper-1&questionId=question-2' };
    global.addEventListener = () => {};
    global.KGQuestionCatalogAdapter = { ready: Promise.resolve() };
    global.localStorage = { getItem(key) { return key === 'kg_exam_papers_published_v1' ? JSON.stringify(published) : null; } };
    global.sessionStorage = { getItem() { return JSON.stringify(staleAttempt); }, removeItem() {} };
    delete require.cache[practicePath];
    require(practicePath);

    await listeners.get('DOMContentLoaded')();
    assert.equal(global.KGPracticeMode.snapshot().active, false);
    assert.equal(global.document.body.dataset.practiceView, 'lobby');
    assert.equal(elementFor('practiceRetiredModeNotice').hidden, false);
  } finally {
    for (const [key, value] of Object.entries(original)) {
      if (value === undefined) delete global[key];
      else global[key] = value;
