...
repo.write('subjects',[{id:'original'}]);
const failed=tx.execute({name:'rollback-test',keys:['subjects'],commit:()=>{repo.write('subjects',[{id:'broken'}]);return {valid:false,errors:['expected failure']}}});
assert(!failed.valid,'transaction must fail');
assert.equal(repo.read('subjects',[])[0].id,'original','failed transaction must restore snapshot');
assert.equal(audit.list()[0].status,'failed','failed transaction must be audited');
...
const first=service.importVersion(payload,{fileName:'first.json'});
...
const second=service.importVersion(payload,{fileName:'second.json'});
...
assert.equal(service.importRecords().length,2,'import records should be stored');
