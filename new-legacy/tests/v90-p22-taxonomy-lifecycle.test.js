...
assert.equal(removedDraft.record.previousStatus,'draft');
assert.equal(removedDraft.record.persisted,false,'deletion result is operation-local until a typed history API exists');
assert.deepEqual(service.deletionRecords(),[],'missing typed deletion history must not be replaced by browser storage');
...
assert(service.releaseRecords().some(item=>item.action==='restore'&&item.taxonomyId===v2.taxonomy.id),'restore history must be derived while the taxonomy exists');
assert((await service.archive(v2.taxonomy.id)).valid);
assert(service.releaseRecords().some(item=>item.action==='archive'&&item.taxonomyId===v2.taxonomy.id),'archive history must be derived while the taxonomy exists');
...
assert.equal(deleted.record.taxonomyId,v2.taxonomy.id);
assert.equal(deleted.record.persisted,false);
assert.deepEqual(service.deletionRecords(),[]);
assert(!service.releaseRecords().some(item=>item.taxonomyId===v2.taxonomy.id),'derived lifecycle rows disappear with a permanently deleted taxonomy');
assert.deepEqual(transactions.snapshots(),[],'generic browser snapshots are retired');
assert.equal(transactions.createSnapshot().valid,false,'generic snapshot creation must be explicitly unsupported');
