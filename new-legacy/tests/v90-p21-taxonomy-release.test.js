...
const imported=service.importVersion({id:'taxonomy-pmp-nine',subjectId:'subject-pmp',name:{zh:'九层测试树'},nodes},{fileName:'nine-level.json'});
...
const tooDeep=service.importVersion({id:'taxonomy-too-deep',subjectId:'subject-cspm',name:{zh:'十层错误树'},nodes:[...nodes,{id:'node-10',parentId:'node-9',level:10,title:{zh:'第10层'}}]},{fileName:'ten-level.json'});
assert(!tooDeep.valid,'level 10 must be rejected');

const denied=service.publish(imported.taxonomy.id,{notes:'teacher cannot publish'});
assert(!denied.valid,'teacher must not publish taxonomies');
assert((denied.errors||[]).some(message=>message.includes('权限')));

role='admin';
const published=service.publish(imported.taxonomy.id,{notes:'管理员确认发布'});
...
const currentSave=service.saveNode(imported.taxonomy.id,{...imported.taxonomy.nodes[0],title:{zh:'当前版本可日常维护'}});
assert(currentSave.valid,'current published taxonomy should support daily maintenance');

const copied=service.createDraftFrom(imported.taxonomy.id);
...
const draftSave=service.saveNode(copied.taxonomy.id,{...copied.taxonomy.nodes[0],title:{zh:'草稿可编辑'}});
assert(draftSave.valid,JSON.stringify(draftSave));

const old=service.list('subject-pmp').find(item=>item.version===1);
const activated=service.publish(old.id,{notes:'回切历史版本'});
...
const historicalSave=service.saveNode(imported.taxonomy.id,{...service.get(imported.taxonomy.id).nodes[0],title:{zh:'历史版本不可修改'}});
