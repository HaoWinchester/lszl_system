#!/usr/bin/env python3
from pathlib import Path
import re
from playwright.sync_api import sync_playwright
ROOT=Path(__file__).resolve().parents[1]
ARGS=['--no-sandbox','--disable-dev-shm-usage','--disable-gpu']
HTML=(ROOT/'admin-console.html').read_text(encoding='utf-8')
BODY=re.search(r'<body[^>]*>([\s\S]*)</body>',HTML,re.I).group(1)
BODY=re.sub(r'<script[\s\S]*?</script>','',BODY,flags=re.I)
SCRIPTS=[
 'src/28-app-storage.js','src/29-auth-core.js','src/86-activity-schema-v1.js','src/87-guided-learning-data.js','src/91-learning-content-core.js','src/93-content-organization-core.js',
 'src/admin/00-admin-core.js','src/admin/10-content-repository.js','src/admin/11-local-content-repository.js','src/admin/20-admin-permission-service.js','src/admin/21-admin-audit-service.js','src/admin/22-admin-transaction-service.js','src/admin/30-reference-index-service.js','src/admin/31-subject-service.js','src/admin/32-taxonomy-service.js','src/admin/33-activity-service.js','src/admin/34-course-service.js','src/admin/35-release-service.js','src/admin/40-admin-service-registry.js','src/admin/41-learning-content-compat.js','src/admin/49-admin-ui.js','src/admin/50-admin-shell-app.js'
]
with sync_playwright() as p:
    browser=p.chromium.launch(headless=True,executable_path='/usr/bin/chromium',args=ARGS)
    page=browser.new_page(viewport={'width':1440,'height':1000})
    errors=[];page.on('pageerror',lambda e:errors.append(str(e)))
    page.set_content('<!doctype html><html><head></head><body>'+BODY+'</body></html>')
    page.add_style_tag(content=(ROOT/'styles/admin-console.css').read_text(encoding='utf-8'))
    page.add_style_tag(content=(ROOT/'styles/admin-context-nav.css').read_text(encoding='utf-8'))
    page.evaluate("""()=>{const local=new Map(),session=new Map();Object.defineProperty(window,'localStorage',{configurable:true,value:{getItem:k=>local.has(k)?local.get(k):null,setItem:(k,v)=>local.set(k,String(v)),removeItem:k=>local.delete(k),clear:()=>local.clear(),key:i=>[...local.keys()][i]||null,get length(){return local.size}}});Object.defineProperty(window,'sessionStorage',{configurable:true,value:{getItem:k=>session.has(k)?session.get(k):null,setItem:(k,v)=>session.set(k,String(v)),removeItem:k=>session.delete(k),clear:()=>session.clear()}});const u='v90-teacher';localStorage.setItem('kg_local_current_user_v1',u);localStorage.setItem('kg_local_users_v1',JSON.stringify({[u]:{username:u,displayName:'V9 测试教师',role:'teacher',status:'active',subject:'PMP',salt:'x',hash:'x'}}));}""")
    for script in SCRIPTS: page.add_script_tag(content=(ROOT/script).read_text(encoding='utf-8'))
    page.wait_for_timeout(250)
    assert page.locator('h1').inner_text()=='管理后台'
    assert page.locator('.admin-context-nav a').count()==9
    assert int(page.locator('#adminSubjectCount').inner_text())>=1
    assert page.locator('#adminRepositoryMode').count()==0
    result=page.evaluate("""()=>KGAdminServices.taxonomies.importVersion({id:'taxonomy-browser',subjectId:'subject-pmp',name:{zh:'浏览器导入树'},nodes:[{id:'browser-root',parentId:null,level:1,title:{zh:'根节点'},status:'active'}]},{fileName:'browser.json'})""")
    assert result['valid'],result
    assert result['taxonomy']['versionLabel']=='v2.0',result
    assert page.evaluate("()=>KGAdminServices.taxonomies.list('subject-pmp').some(item=>item.versionLabel==='v2.0')")
    assert page.evaluate('()=>!!window.KGLearningContent.adminServices')
    assert not errors,errors
    browser.close()
print('v90-p0-admin-shell-browser-ok')
