...
const repo=new W.KGLocalContentRepository();repo.write('subjects',subjects);
...
const created=service.create({code:'NEW',nameZh:'新科目',nameEn:'New Subject',descriptionZh:'用于测试'});
...
assert(!service.create({code:'NEW',nameZh:'重复科目'}).valid,'duplicate code must be rejected');
const updated=service.update(created.subject.id,{nameZh:'新科目（已编辑）',nameEn:'Edited',descriptionZh:'已更新说明'});
...
const moved=service.move(created.subject.id,'up');assert(moved.valid,'subject should move up');assert(service.list().findIndex(item=>item.id===created.subject.id)<service.list().findIndex(item=>item.id==='subject-empty'));
const stopped=service.setStatus(created.subject.id,'inactive');assert(stopped.valid);assert(service.isInactive(stopped.subject));
const restored=service.setStatus(created.subject.id,'active');assert(restored.valid);assert(!service.isInactive(restored.subject));
const protectedDelete=service.delete('subject-used');assert(!protectedDelete.valid,'used subject must not be deleted');assert(protectedDelete.usage.counts.taxonomy===1);assert(protectedDelete.usage.counts.activity===1);assert(protectedDelete.usage.counts.paper===1);
const emptyDelete=service.delete('subject-empty');assert(emptyDelete.valid,'empty subject should be deleted');assert(!service.get('subject-empty'));
