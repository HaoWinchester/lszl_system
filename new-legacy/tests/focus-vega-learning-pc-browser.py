...
def inspect(page, filename, spec, width, height):
    result = page.evaluate(
        """({roots,panelSelectors,primarySelector}) => {
          ...
          const iconOnly='button[aria-label]:not(:has(span:not([aria-hidden="true"]))):not(:has(strong)),.gl-node-button';
    ...
    if result['bodyBackground'] != 'rgb(250, 250, 250)':
...
def main():
    ...
    if args.group == 'chrome':
        print('focus-vega-learning-pc-browser-ok 0/0')
        return
    checked = 0
    with sync_playwright() as playwright:
        browser = playwright.chromium.launch(headless=True, args=ARGS)
        try:
            for filename, spec in STANDALONE.items():
            ...
            browser.close()
    print(f'focus-vega-learning-pc-browser-ok {checked}/{len(STANDALONE)*len(VIEWPORTS)}')
