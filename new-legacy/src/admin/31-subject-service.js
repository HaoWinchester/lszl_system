...
(function(global){
  ...
  class SubjectService{
    ...
    async create(input={}){
      ...
      const tx=await this.transactions.executeAsync({name:'新增科目',action:'subject.create',entityType:'subject',entityId:record.id,permission:'editSubjects',keys:['subjects'],validate:()=>validation,commit:async()=>{await this.legacy.saveSubjects(next);return {valid:true,subject:this.legacy.subjectById(record.id)}},summary:`新增科目：${record.name.zh}`,metadata:{after:record}});
    ...
    async update(subjectId,input={}){
      ...
      const tx=await this.transactions.executeAsync({name:'编辑科目',action:'subject.update',entityType:'subject',entityId:subjectId,permission:'editSubjects',keys:['subjects'],validate:()=>validation,commit:async()=>{await this.legacy.saveSubjects(next);return {valid:true,subject:this.legacy.subjectById(subjectId)}},summary:`编辑科目：${after.name.zh}`,metadata:{before,after}});
      if(tx.valid)this.references?.invalidate();return tx.valid?{valid:true,subject:tx.value.subject,transactionId:tx.transactionId,snapshotId:tx.snapshotId,errors:[],warnings:[]}:{valid:false,errors:tx.errors||[]};
    }
    async move(subjectId,direction){
      ...
      const tx=await this.transactions.executeAsync({name:'调整科目顺序',action:'subject.reorder',entityType:'subject',entityId:subjectId,permission:'editSubjects',keys:['subjects'],validate:()=>this.validate(next),commit:async()=>{await this.legacy.saveSubjects(next);return {valid:true,subjects:this.legacy.getSubjects()}},summary:`调整科目顺序：${this.get(subjectId)?.name?.zh||subjectId}`,metadata:{direction:delta<0?'up':'down',before,after:next.map(item=>({id:item.id,sortOrder:item.sortOrder}))}});
    ...
    async setStatus(subjectId,status){
      ...
      const tx=await this.transactions.executeAsync({name,action,entityType:'subject',entityId:subjectId,permission:'editSubjects',keys:['subjects'],validate:()=>this.validate(next),commit:async()=>{await this.legacy.saveSubjects(next);return {valid:true,subject:this.legacy.subjectById(subjectId)}},summary:`${name}：${before.name?.zh||before.code}`,metadata:{before,after}});
    ...
    async delete(subjectId){
      ...
      const tx=await this.transactions.executeAsync({name:'永久删除空科目',action:'subject.delete',entityType:'subject',entityId:subjectId,permission:'editSubjects',keys:['subjects'],validate:()=>this.validate(next),commit:async()=>{await this.legacy.saveSubjects(next);return {valid:true,subjects:this.legacy.getSubjects()}},summary:`永久删除空科目：${before.name?.zh||before.code}`,metadata:{before,usage:check.usage}});
    ...
    async saveAll(subjects){const incoming=Array.isArray(subjects)?subjects:[],current=this.list(),incomingIds=new Set(incoming.map(item=>item.id));const tx=await this.transactions.executeAsync({name:'保存科目',action:'subject.save',entityType:'subject',permission:'editSubjects',keys:['subjects'],validate:()=>{const validation=this.validate(incoming),removed=current.filter(item=>!incomingIds.has(item.id));if(removed.length){const authority=global.KGReferenceIndexService.permanentDeleteAuthority(this.references);if(!authority.valid)return {valid:false,errors:authority.errors||['永久删除科目已暂停。'],removedSubjectIds:removed.map(item=>item.id)}}return validation},commit:async()=>{await this.legacy.saveSubjects(incoming);return {valid:true,subjects:this.legacy.getSubjects()}},metadata:{count:incoming.length}});if(tx.valid)this.references?.invalidate();return tx.valid?{valid:true,subjects:tx.value.subjects,transactionId:tx.transactionId,errors:[],warnings:[]}:{valid:false,errors:tx.errors||[]}}
