...
(function(global){
  ...
  class TaxonomyService{
    ...
    importVersion(payload,options={}){
      ...
      const all=this.list(),tx=this.transactions.execute({name:'导入知识树版本',action:'taxonomy.import',entityType:'taxonomy',entityId:id,permission:'importTaxonomies',keys:['taxonomies','taxonomyImports'],validate:()=>validation,commit:()=>{const saved=this.legacy.saveTaxonomies([...all,imported]);if(!saved.valid)return saved;const records=this.repository.read('taxonomyImports',[]),record={id:Core.safeId('taxonomy-import'),importedAt:now,importedBy:actor,fileName:Core.clean(options.fileName),subjectId:parsed.subjectId,taxonomyId:id,version,versionLabel:this.versionLabel(version),nodeCount:imported.nodes.length,result:'success'};this.repository.write('taxonomyImports',[record,...(Array.isArray(records)?records:[])].slice(0,100));return {valid:true,taxonomy:this.get(id),importRecord:record}},metadata:{subjectId:parsed.subjectId,version,nodeCount:imported.nodes.length,fileName:Core.clean(options.fileName)}});
    ...
    publish(taxonomyId,options={}){
      ...
      const tx=this.transactions.execute({name:releaseType==='publish'?'发布知识树为当前':'切换当前知识树',action:releaseType==='publish'?'taxonomy.publish':'taxonomy.activate',entityType:'taxonomy',entityId:taxonomyId,permission:'publishTaxonomies',keys:['subjects','taxonomies','taxonomyReleases'],validate:()=>check,commit:()=>{
        ...
        const saved=this.legacy.saveTaxonomies(taxonomies);if(!saved.valid)return saved;
        const subjects=this.legacy.getSubjects().map(item=>item.id===source.subjectId?{...item,defaultTaxonomyId:taxonomyId}:item);this.legacy.saveSubjects(subjects);
        const records=this.releaseRecords(),record={id:Core.safeId('taxonomy-release'),action:releaseType,subjectId:source.subjectId,taxonomyId,version:Number(source.version)||1,versionLabel:source.versionLabel||this.versionLabel(source.version),previousTaxonomyId:current?.id||'',at:now,actor,notes,nodeCount:check.nodeCount,maxLevel:check.maxLevel,contentHash:Core.hash(JSON.stringify(source))};this.repository.write('taxonomyReleases',[record,...records].slice(0,200));return {valid:true,taxonomy:this.get(taxonomyId),record};
    ...
    createDraftFrom(taxonomyId){const source=this.get(taxonomyId);if(!source)return {valid:false,errors:['知识树不存在。']};const version=this.nextVersion(source.subjectId),id=this.uniqueTaxonomyId(source.id,version),now=Core.nowIso(),actor=Core.actor(),draft={...Core.clone(source),id,version,versionLabel:this.versionLabel(version),status:'draft',isDefault:false,maxDepth:Number(this.legacy.MAX_DEPTH)||9,createdAt:now,updatedAt:now,createdBy:actor,publishedAt:'',activatedAt:'',publishedBy:null,activatedBy:null,releaseNotes:'',archivedAt:'',archivedBy:null,source:{type:'copy',sourceTaxonomyId:source.id,sourceVersion:source.version,copiedAt:now}};draft.nodes=(draft.nodes||[]).map(item=>({...item,taxonomyId:id}));const validation=this.validate(draft);if(!validation.valid)return validation;const tx=this.transactions.execute({name:'基于版本创建知识树草稿',action:'taxonomy.draft.create',entityType:'taxonomy',entityId:id,permission:'editTaxonomies',keys:['taxonomies'],validate:()=>validation,commit:()=>{const saved=this.legacy.saveTaxonomies([...this.list(),draft]);return saved.valid?{valid:true,taxonomy:this.get(id)}:saved},metadata:{subjectId:source.subjectId,sourceTaxonomyId:source.id,version}});if(tx.valid)this.references?.invalidate();return tx.valid?{valid:true,taxonomy:tx.value.taxonomy,transactionId:tx.transactionId,errors:[],warnings:validation.warnings||[]}:{valid:false,errors:tx.errors||[]}}
    ...
    archive(taxonomyId,options={}){
      ...
      const tx=this.transactions.execute({name:'归档历史知识树',action:'taxonomy.archive',entityType:'taxonomy',entityId:taxonomyId,permission:'deleteTaxonomies',keys:['taxonomies','taxonomyReleases'],commit:()=>{
        const rows=this.list().map(item=>item.id===taxonomyId?{...item,status:'archived',isDefault:false,archivedAt:now,archivedBy:actor,archiveNotes:notes,updatedAt:now}:item),saved=this.legacy.saveTaxonomies(rows);if(!saved.valid)return saved;
        const records=this.releaseRecords(),record={id:Core.safeId('taxonomy-release'),action:'archive',subjectId:source.subjectId,taxonomyId,version:Number(source.version)||1,versionLabel:source.versionLabel||this.versionLabel(source.version),previousTaxonomyId:'',at:now,actor,notes,nodeCount:(source.nodes||[]).length,maxLevel:Math.max(0,...(source.nodes||[]).map(item=>Number(item.level)||0)),contentHash:Core.hash(JSON.stringify(source))};this.repository.write('taxonomyReleases',[record,...records].slice(0,200));return {valid:true,taxonomy:this.get(taxonomyId),record};
    ...
    restoreArchived(taxonomyId){
      const permission=this.permissions?.require?.('deleteTaxonomies')||{valid:true};if(!permission.valid)return permission;
      const source=this.get(taxonomyId);if(!source)return {valid:false,errors:['知识树不存在。']};
      if(source.status!=='archived')return {valid:false,errors:['只有已归档知识树可以恢复。']};
      const now=Core.nowIso(),actor=Core.actor();
      const tx=this.transactions.execute({name:'恢复归档知识树',action:'taxonomy.archive.restore',entityType:'taxonomy',entityId:taxonomyId,permission:'deleteTaxonomies',keys:['taxonomies','taxonomyReleases'],commit:()=>{
        const rows=this.list().map(item=>item.id===taxonomyId?{...item,status:'published',isDefault:false,restoredAt:now,restoredBy:actor,updatedAt:now}:item),saved=this.legacy.saveTaxonomies(rows);if(!saved.valid)return saved;
        const records=this.releaseRecords(),record={id:Core.safeId('taxonomy-release'),action:'restore',subjectId:source.subjectId,taxonomyId,version:Number(source.version)||1,versionLabel:source.versionLabel||this.versionLabel(source.version),previousTaxonomyId:'',at:now,actor,notes:'',nodeCount:(source.nodes||[]).length,maxLevel:Math.max(0,...(source.nodes||[]).map(item=>Number(item.level)||0)),contentHash:Core.hash(JSON.stringify(source))};this.repository.write('taxonomyReleases',[record,...records].slice(0,200));return {valid:true,taxonomy:this.get(taxonomyId),record};
      },metadata:{subjectId:source.subjectId,version:source.version}});
      if(tx.valid)this.references?.invalidate();
      return tx.valid?{valid:true,taxonomy:tx.value.taxonomy,record:tx.value.record,transactionId:tx.transactionId,errors:[]}:{valid:false,errors:tx.errors||[]};
    }
    deleteVersion(taxonomyId,options={}){
      ...
      const tx=this.transactions.execute({name:source.status==='draft'?'删除知识树草稿':'永久删除归档知识树',action:'taxonomy.delete',entityType:'taxonomy',entityId:taxonomyId,permission:'deleteTaxonomies',keys:['taxonomies','taxonomyDeletions'],validate:()=>this.deletionCheck(taxonomyId),commit:()=>{
        const saved=this.legacy.saveTaxonomies(this.list().filter(item=>item.id!==taxonomyId));if(!saved.valid)return saved;
        const records=this.deletionRecords(),record={id:Core.safeId('taxonomy-deletion'),deletedAt:now,deletedBy:actor,subjectId:source.subjectId,taxonomyId:source.id,name:Core.clone(source.name||{}),version:Number(source.version)||1,versionLabel:source.versionLabel||this.versionLabel(source.version),previousStatus:source.status,nodeCount:(source.nodes||[]).length,notes,contentHash:Core.hash(JSON.stringify(source))};this.repository.write('taxonomyDeletions',[record,...records].slice(0,200));return {valid:true,record};
    ...
    saveAll(taxonomies){
      ...
      const tx=this.transactions.execute({name:'保存知识树',action:'taxonomy.save',entityType:'taxonomy',permission:'editTaxonomies',keys:['taxonomies'],validate:()=>{
        const errors=[];if(incoming.length!==current.length)errors.push('知识树整体恢复不能新增或删除版本。');
        incoming.forEach(item=>{const before=existingById.get(item.id),check=this.validate(item);check.errors.forEach(message=>errors.push(`${item.id||'未命名知识树'}：${message}`));if(!before){errors.push(`知识树版本不存在：${item.id}`);return}if(before.subjectId!==item.subjectId||Number(before.version)!==Number(item.version)||before.status!==item.status||!!before.isDefault!==!!item.isDefault)errors.push(`${item.id} 的版本身份和生命周期状态不能通过日常编辑修改。`);if(JSON.stringify(before)!==JSON.stringify(item)&&!this.editMode(before))errors.push(`${item.id} 是历史版本，只能查看。`) });
    ...
    restoreNode(taxonomyId,nodeId,patch={}){
    ...
    deleteNode(taxonomyId,nodeId,options={}){
      ...
      const tx=this.transactions.execute({name:'删除空知识点',action:'taxonomy.node.delete',entityType:'knowledge-node',entityId:nodeId,permission:'editTaxonomies',keys:['taxonomies'],validate:()=>this.nodeDeletionCheck(taxonomyId,nodeId),commit:()=>this.legacy.deleteKnowledgeNode(taxonomyId,nodeId,{cascade:false}),metadata:{taxonomyId,subjectId:check.taxonomy.subjectId,mode:check.mode,before:Core.clone(node),after:null,deletedAt:now,deletedBy:actor,cascadeRequested:!!options.cascade}});
      if(tx.valid)this.references?.invalidate();return tx.valid?{...tx.value,transactionId:tx.transactionId,snapshotId:tx.snapshotId}:{valid:false,errors:tx.errors||[]}
    }
    importRecords(){const rows=this.repository.read('taxonomyImports',[]);return Array.isArray(rows)?Core.clone(rows):[]}
    releaseRecords(subjectId=''){const rows=this.repository.read('taxonomyReleases',[]),list=Array.isArray(rows)?rows:[];return Core.clone(subjectId?list.filter(item=>item.subjectId===subjectId):list)}
    deletionRecords(subjectId=''){const rows=this.repository.read('taxonomyDeletions',[]),list=Array.isArray(rows)?rows:[];return Core.clone(subjectId?list.filter(item=>item.subjectId===subjectId):list)}
  }
