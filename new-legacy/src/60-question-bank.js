...
try{window.addEventListener('kg:published-papers-changed',qbInvalidateCaches);window.addEventListener('kg:question-catalog-changed',qbInvalidateCaches)}catch(e){}
function qbLoadPapers(){
  ...
  try{
    ...
    published=repository?.listReleases?.({includeHistory:true})||[];
  }catch(e){published=[]}
  qBankState.papers=(Array.isArray(published)?published:[]).map(qbNormalizePaper).filter(p=>p.status==='published'&&p.availability!=='withdrawn');
...
function qbPaperQuestionByRef(ref,paper=null){
  ...
  const banks=qbLoadBanks();const bank=banks.find(b=>b.id===ref.bankId);const question=bank&&(bank.questions||[]).find(q=>q.id===ref.questionId);
...
function qbPublishedPaperCatalog(options={}){
  ...
  if(repository){
    ...
    const entries=(repository.listCatalogEntries(options)||[]).map(catalog=>{
      ...
      if(cached)return cached;
      return {ok:true,paper:{...catalog,id:catalog.paperId,availability:'published'},items:[],configuredCount:catalog.totalCount,targetCount:catalog.totalCount,availableCount:0,missingCount:0,damagedCount:0,blockedCount:0,issues:[]};
    });
...
function qbSaveCurrentPaper(){return true}
...
function qbLoadBanks(){
  ...
  if(!QB_CATALOG?.snapshot){showStatus('题目目录 API 未加载，请刷新后重试。');qBankState.banks=[];return qBankState.banks}
...
function qbSaveBanks(){showStatus('正式题库由服务器统一管理，请在题库管理页完成修改。');return false}
function qbSaveCurrent(){return true}
...
function qbImportJsonFile(file){
  if(!authRequire('登录后才能导入题库。')||!file)return Promise.resolve();
  return globalThis.KGImportGuard.run('home-bank-import',async()=>{try{const raw=String(await file.text()).replace(/^\ufeff/,'');const data=JSON.parse(raw);const bankData=Array.isArray(data.questions)?data:(data.bankName||data.name?data:null);if(!bankData)throw new Error('JSON 中未找到 questions 数组。');const errors=qbValidateBank(bankData);if(errors.length)throw new Error(errors.slice(0,5).join('\n'));if(!QB_CATALOG?.importBanks)throw new Error('题库 API 未加载，请刷新后重试。');const result=await QB_CATALOG.importBanks({banks:[bankData],confirmReplace:false,confirmDuplicateCleanup:false});await QB_CATALOG.reload?.({source:'question-import',includeQuestions:true});qbInvalidateCaches();qbLoadBanks();const bank=result?.banks?.[0];if(bank){qBankState.selectedBankId=bank.id;qBankState.selectedQuestionId=bank.questions?.[0]?.id||null}renderQuestionBankManager();showStatus(`已导入题库：${bank?.name||bankData.name}，共 ${bank?.questions?.length||bankData.questions.length} 题。`)}catch(err){alert('导入失败：\n'+(err.message||err))}
  },['qbImportBtn','qbImportFile']);
}
