...
/*
 * CanvasWorkspaceStore v6
 * 多题长期工作区。单题 LearningSession 与跨题 CanvasWorkspace 完全分离。
 */
(function(global){
  const STORAGE_PREFIX='kg_canvas_workspace_v1__';
  ...
  function questionIdentity(questionId,bankId=''){
    return String(bankId||'')+'::'+String(questionId||'');
  }
  function questionNodeId(questionId,bankId=''){
    return 'question-ref-'+hashString(questionIdentity(questionId,bankId));
  }
  ...
  function normalizeNode(node={}){
    ...
    if(nodeType==='synthesis-card'){
      ...
      return {
        ...
        color:GROUP_COLORS.includes(String(node.color||''))?String(node.color):'#ede9fe',
    ...
    return {
      ...
      color:GROUP_COLORS.includes(String(node.color||''))?String(node.color):'',
  ...
  function normalizeGroup(group={},index=0){
    return {
      ...
      color:GROUP_COLORS.includes(String(group.color||''))?String(group.color):'#ede9fe',
  ...
  function sessionStatus(questionId){
    const session=global.KGLearningSessionStore?.get?.(String(questionId||''),currentUserId())||null;
  ...
  function snapshotQuestion(question={},bankId='',position={}){
    ...
    const progress=sessionStatus(questionId);
    return normalizeNode({
      id:questionNodeId(questionId,bankId),
      nodeType:'question-reference',
      questionId,
      bankId:String(bankId||question.sourceBankId||''),
      paperId:String(question.sourcePaperId||''),
      releaseId:String(question.sourceReleaseId||''),
  ...
  function findQuestionNode(questionId,bankId='',options={}){
    const workspace=ensure(options);
    const identity=questionIdentity(questionId,bankId);
    return Object.values(workspace.nodes).find(node=>
      node.nodeType==='question-reference'&&questionIdentity(node.questionId,node.bankId)===identity
    )||null;
  }
  function addQuestionReference(question={},bankId='',position={},options={}){
    ...
    const existing=Object.values(workspace.nodes).find(node=>
      node.nodeType==='question-reference'&&questionIdentity(node.questionId,node.bankId)===questionIdentity(questionId,resolvedBankId)
    )||null;
    if(existing){
  ...
  function removeNode(nodeId,options={}){
    ...
    const saved=update(workspace=>{
      ...
      workspace.edges=(workspace.edges||[]).filter(edge=>edge.source!==nodeId&&edge.target!==nodeId);
  ...
  function refreshQuestionProgress(questionId,options={}){
    const progress=sessionStatus(questionId);
    const workspace=ensure(options);
    let changed=false;
    Object.values(workspace.nodes).forEach(node=>{
      if(node.questionId!==String(questionId||''))return;
