...
(function(){
  ...
  let toastTimer,scrollSaveTimer,recentEditTimer,persistQueue=Promise.resolve();
  ...
  function readRecent(){const parsed=Preferences?.getJSON?.(RECENT_KEY,[]);return Array.isArray(parsed)?parsed:[]}
  function writeRecent(items){Preferences?.setJSON?.(RECENT_KEY,items.slice(0,30));return true}
  ...
  function readWorkspaceMap(){return Preferences?.getJSON?.(WORKSPACE_KEY,{})||{}}
  function writeWorkspaceMap(map){Preferences?.setJSON?.(WORKSPACE_KEY,map);return true}
  ...
  async function applyTemplate(){
    ...
    if(!added.length)return toast('所选章节已经包含该模板的全部步骤。');normalizeOrders();await persist();const first=added[0];state.selection={kind:'node',id:first.id};ensureSelectionVisible();recordRecent(`套用模板：${template.name}`,state.selection);renderAll();toast(`已向 ${targets.length} 个章节新增 ${added.length} 个学习步骤。`);
  }
  async function cloneCurrentStructure(){
    ...
    normalizeOrders();await persist();if(firstSelection){state.selection=firstSelection;ensureSelectionVisible();recordRecent(kind==='part'?'批量复制章节':'批量复制阶段',firstSelection)}renderAll();toast(`已生成 ${count} 份副本，共新增 ${createdCount} 个课程项。`);
  }
  ...
  async function applyOutline(){
    ...
    normalizeOrders();await persist();if(firstSelection){state.selection=firstSelection;ensureSelectionVisible();recordRecent('按大纲生成课程结构',firstSelection)}renderAll();$('caBatchToolsDialog')?.close();toast(`已按大纲新增 ${created} 个课程项。`);
  }
  ...
  async function persist(silent=true){const c=course();if(!c)return null;const selectedId=c.id,payload=Core.clone(c);const operation=persistQueue.catch(()=>{}).then(async()=>{const saved=await Core.saveCourseDraft(payload);state.courses=Core.getCourseDrafts();if(state.courseId===selectedId)state.courseId=saved.id;saveWorkspaceState();if(!silent)toast('课程草稿已保存。');renderPreview();return saved}).catch(error=>{toast(error?.status===409?'课程已被其他人更新，已刷新最新内容，请确认后重试。':`课程保存失败：${error?.message||error}`);state.courses=Core.getCourseDrafts();renderAll();return null});persistQueue=operation;return operation}
  async function addStage(){const c=course();if(!c)return;const item={id:Core.safeId('stage'),title:`新阶段 ${c.stages.length+1}`,order:c.stages.length+1};c.stages.push(item);state.selection={kind:'stage',id:item.id};state.currentStageId=item.id;state.expandedStages.add(item.id);await persist();recordRecent('新增阶段',state.selection);renderAll()}
  async function addPart(){const c=course(),s=stage(state.selection.id)||stage(state.currentStageId);if(!c||!s)return;const count=c.parts.filter(item=>item.stageId===s.id).length;const item={id:Core.safeId('part'),stageId:s.id,title:`新章节 ${count+1}`,order:count+1};c.parts.push(item);state.selection={kind:'part',id:item.id};state.currentStageId=s.id;state.expandedParts.clear();state.expandedParts.add(item.id);await persist();recordRecent('新增章节',state.selection);renderAll()}
  async function addNode(){const c=course(),p=part(state.selection.id);if(!c||!p)return;const count=c.nodes.filter(item=>item.partId===p.id).length;const item={id:Core.safeId('node'),partId:p.id,title:`新学习步骤 ${count+1}`,order:count+1,nodeType:'standard',activityIds:[],description:'',settings:{}};c.nodes.push(item);state.selection={kind:'node',id:item.id};state.expandedParts.clear();ensureSelectionVisible();await persist();recordRecent('新增学习步骤',state.selection);renderAll()}
  async function deleteSelection(){
    ...
    chooseInitialSelection();await persist();renderAll();
  }
  async function newCourse(){const preferred=Core.subjectById(Preferences?.getString?.(SUBJECT_PREF_KEY,'')),subject=preferred||subjects()[0],taxonomy=Core.defaultTaxonomyForSubject(subject.id);try{const saved=await Core.saveCourseDraft(Core.normalizeCourse({name:'新课程',subjectId:subject.id,taxonomyId:taxonomy?.id||'',description:'',stages:[],parts:[],nodes:[]}));state.courses=Core.getCourseDrafts();state.courseId=saved.id;state.expandedStages.clear();state.expandedParts.clear();state.selection={kind:'course',id:saved.id};state.currentStageId='';state.treeScrollTop=0;renderAll();saveWorkspaceState();toast('新课程草稿已创建。')}catch(error){toast(`创建课程失败：${error?.message||error}`)}}
  async function deleteCourse(){if(state.courses.length<=1)return toast('至少保留一个课程草稿。');if(!confirm('删除当前课程草稿？已发布版本不会删除。'))return;try{await Core.deleteCourseDraft(state.courseId);state.courses=Core.getCourseDrafts();state.courseId=state.courses[0]?.id||'';state.expandedStages.clear();state.expandedParts.clear();state.treeScrollTop=0;if(!restoreWorkspaceState())chooseInitialSelection();renderAll()}catch(error){toast(error?.status===409?'课程已被更新，已刷新，请确认后重试。':`删除失败：${error?.message||error}`)}}
  async function openPublish(){const saved=await persist();if(!saved)return;const result=Core.validateCourse(course());$('caPublishWarnings').innerHTML=[...result.errors.map(item=>`<p class="validation-error">错误：${escapeHTML(item)}</p>`),...result.warnings.map(item=>`<p class="validation-warn">提醒：${escapeHTML(item)}</p>`)].join('')||'<p class="validation-ok">课程结构校验通过，可以发布。</p>';$('caConfirmPublishBtn').disabled=!result.valid;$('caPublishDialog').showModal()}
  async function publish(){try{const result=await Core.publishCourse(state.courseId,$('caPublishNotes').value);if(!result.valid)return toast('发布失败：'+(result.errors||[]).join('；'));state.courses=Core.getCourseDrafts();$('caPublishDialog').close();$('caPublishNotes').value='';renderPreview();toast(`课程 v${result.release.version} 已发布。`)}catch(error){toast(error?.status===409?'课程已被更新，已刷新，请确认后重试。':`发布失败：${error?.message||error}`)}}
  async function exportCourse(){const saved=await persist();if(!saved)return;const c=course(),payload={schemaVersion:1,exportedAt:new Date().toISOString(),course:c,knowledgeCoverage:Core.courseKnowledgeCoverage(c)},blob=new Blob([JSON.stringify(payload,null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=`${c.id}-draft.json`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),500)}

  function bind(){
    $('caCourseSelect').addEventListener('change',async event=>{await persist();state.courseId=event.target.value;state.expandedStages.clear();state.expandedParts.clear();state.treeScrollTop=0;if(!restoreWorkspaceState())chooseInitialSelection();renderAll()});
    $('caCourseName').addEventListener('input',async event=>{course().name=event.target.value;await persist();scheduleRecent('编辑课程信息');renderStructure()});$('caDescription').addEventListener('input',async event=>{course().description=event.target.value;await persist();scheduleRecent('编辑课程信息')});
    $('caSubject').addEventListener('change',async event=>{course().subjectId=event.target.value;Preferences?.setString?.(SUBJECT_PREF_KEY,event.target.value);course().taxonomyId=Core.defaultTaxonomyForSubject(event.target.value)?.id||'';state.knowledgeId='';await persist();renderAll()});$('caTaxonomy').addEventListener('change',async event=>{course().taxonomyId=event.target.value;state.knowledgeId='';await persist();renderKnowledgeFilter();renderPicker();renderPreview()});
    ...
    $('caNodeFields').addEventListener('input',async event=>{const key=event.target.dataset.edit;if(!key)return;const item=selected();item[key]=key==='order'?Number(event.target.value)||1:event.target.value;await persist();scheduleRecent('编辑课程结构');renderStructure();renderPreview()});$('caNodeFields').addEventListener('change',async event=>{const key=event.target.dataset.edit;if(!key)return;selected()[key]=event.target.value;await persist();recordRecent('编辑课程结构');renderStructure();renderPreview()});
    $('caAddStageBtn').addEventListener('click',async()=>{await addStage()});$('caAddPartBtn').addEventListener('click',async()=>{await addPart()});$('caAddNodeBtn').addEventListener('click',async()=>{await addNode()});$('caDeleteItemBtn').addEventListener('click',async()=>{await deleteSelection()});$('caNewCourseBtn').addEventListener('click',async()=>{await newCourse()});$('caDeleteCourseBtn').addEventListener('click',async()=>{await deleteCourse()});$('caSaveBtn').addEventListener('click',async()=>{await persist(false)});$('caExportBtn').addEventListener('click',async()=>{await exportCourse()});$('caPublishBtn').addEventListener('click',async()=>{await openPublish()});$('caConfirmPublishBtn').addEventListener('click',async()=>{await publish()});
    $('caAssignedActivities').addEventListener('click',async event=>{const n=selectedNode();if(!n)return;const remove=event.target.closest('[data-remove-activity]');if(remove){n.activityIds=n.activityIds.filter(id=>id!==remove.dataset.removeActivity);await persist();recordRecent('移除学习内容');renderEditor();renderPicker();renderStructure();renderPreview();return}const move=event.target.closest('[data-move]');if(move){const index=Number(move.dataset.index),target=index+Number(move.dataset.move);if(target>=0&&target<n.activityIds.length)[n.activityIds[index],n.activityIds[target]]=[n.activityIds[target],n.activityIds[index]];await persist();recordRecent('调整内容顺序');renderEditor();renderPreview()}});
    $('caActivityPicker').addEventListener('click',async event=>{const button=event.target.closest('[data-add-activity]'),n=selectedNode();if(!button||!n)return;if(!n.activityIds.includes(button.dataset.addActivity))n.activityIds.push(button.dataset.addActivity);await persist();recordRecent('加入题目或训练');renderEditor();renderPicker();renderStructure();renderPreview()});$('caActivitySearch').addEventListener('input',event=>{state.activityQuery=event.target.value;renderPicker();saveWorkspaceState()});$('caKnowledgeFilter').addEventListener('change',event=>{state.knowledgeId=event.target.value;renderPicker();saveWorkspaceState()});$('caOnlyMapped').addEventListener('change',event=>{state.onlyMapped=event.target.checked;renderPicker();saveWorkspaceState()});
    ...
    $('caTemplateSelect')?.addEventListener('change',updateTemplateDescription);$('caCopyKind')?.addEventListener('change',updateCopyTargetVisibility);$('caApplyTemplateBtn')?.addEventListener('click',async()=>{await applyTemplate()});$('caCopyStructureBtn')?.addEventListener('click',async()=>{await cloneCurrentStructure()});$('caParseOutlineBtn')?.addEventListener('click',parseOutlinePreview);$('caApplyOutlineBtn')?.addEventListener('click',async()=>{await applyOutline()});
  ...
  async function init(){
    try{await Promise.all([window.KGTeachingContentApi?.bootstrap?.(),window.KGCourseManagementApi?.ready?.()]);state.courses=Core.getCourseDrafts();state.courseId=state.courses[0]?.id||'';if(!restoreWorkspaceState())chooseInitialSelection();const current=course();if(current)Preferences?.setString?.(SUBJECT_PREF_KEY,current.subjectId);bind();renderAll()}catch(error){toast(`课程数据加载失败：${error?.message||error}`)}
  }
