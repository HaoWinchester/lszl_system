...
(function(){
  ...
  const DraftSaver=window.KGCourseManagementApi?.createDraftSaveQueue?.();
  ...
  async function applyTemplate(){
    ...
    if(!added.length)return toast('所选章节已经包含该模板的全部步骤。');normalizeOrders();if(!await persist())return;const first=added[0];state.selection={kind:'node',id:first.id};ensureSelectionVisible();recordRecent(`套用模板：${template.name}`,state.selection);renderAll();toast(`已向 ${targets.length} 个章节新增 ${added.length} 个学习步骤。`);
  }
  async function cloneCurrentStructure(){
    ...
    normalizeOrders();if(!await persist())return;if(firstSelection){state.selection=firstSelection;ensureSelectionVisible();recordRecent(kind==='part'?'批量复制章节':'批量复制阶段',firstSelection)}renderAll();toast(`已生成 ${count} 份副本，共新增 ${createdCount} 个课程项。`);
  }
  ...
  async function applyOutline(){
    ...
    normalizeOrders();if(!await persist())return;if(firstSelection){state.selection=firstSelection;ensureSelectionVisible();recordRecent('按大纲生成课程结构',firstSelection)}renderAll();$('caBatchToolsDialog')?.close();toast(`已按大纲新增 ${created} 个课程项。`);
  }
  ...
  async function deleteSelection(){
    ...
    chooseInitialSelection();if(!await persist())return;renderAll();
  }
  ...
  function bind(){
    $('caCourseSelect').addEventListener('change',async event=>{if(!await persist())return;state.courseId=event.target.value;state.expandedStages.clear();state.expandedParts.clear();state.treeScrollTop=0;if(!restoreWorkspaceState())chooseInitialSelection();renderAll()});
    $('caCourseName').addEventListener('input',async event=>{course().name=event.target.value;if(!await persist())return;scheduleRecent('编辑课程信息');renderStructure()});$('caDescription').addEventListener('input',async event=>{course().description=event.target.value;if(!await persist())return;scheduleRecent('编辑课程信息')});
    $('caSubject').addEventListener('change',async event=>{course().subjectId=event.target.value;Preferences?.setString?.(SUBJECT_PREF_KEY,event.target.value);course().taxonomyId=Core.defaultTaxonomyForSubject(event.target.value)?.id||'';state.knowledgeId='';if(!await persist())return;renderAll()});$('caTaxonomy').addEventListener('change',async event=>{course().taxonomyId=event.target.value;state.knowledgeId='';if(!await persist())return;renderKnowledgeFilter();renderPicker();renderPreview()});
    ...
    $('caNodeFields').addEventListener('input',async event=>{const key=event.target.dataset.edit;if(!key)return;const item=selected();item[key]=key==='order'?Number(event.target.value)||1:event.target.value;if(!await persist())return;scheduleRecent('编辑课程结构');renderStructure();renderPreview()});$('caNodeFields').addEventListener('change',async event=>{const key=event.target.dataset.edit;if(!key)return;selected()[key]=event.target.value;if(!await persist())return;recordRecent('编辑课程结构');renderStructure();renderPreview()});
