...
(function(){
  ...
  function parsedToQuestion(result){
    ...
    const question={title:result.title||plain.slice(0,48)+(plain.length>48?'…':''),type:'single_choice',subject:resolution?.subjectCode||'',difficulty:'中等',domain:'',topic:result.topic||'',tags:unique(resolution?.tags||result.tags||[]),stemParts:P2?.markStemParts?.(result.stem||result.stemEn,keywords,'zh')||[{text:result.stem||result.stemEn}],options:result.options.map(option=>({id:option.id,text:option.text||option.textEn,trap:'',correct:option.id===result.answer})),correctAnswer:result.answer,analysis:result.analysis||'',clues,concepts:[],reasoningSteps:[],metadata:{translationStatus:result.language,subjectId:resolution?.subjectId||'',knowledge,classificationImport:{subject:result.subject||'',knowledge:result.knowledge||'',matchStatus:resolution?.matchStatus||'empty',importedAt:new Date().toISOString()},bilingualKeywordMappings:keywords.map(item=>({zh:item.zh,en:item.en,entry:item.entry}))},status:{contentReady:true,keywordsReady:clues.length>0,knowledgeReady:!!knowledge.primaryNodeId,reasoningReady:false,published:false}};
  ...
  function applySingleQuestion(){
    ...
    const saved=placeholder?api.updateCurrentQuestion?.(question):api.bulkAddQuestions?.([question],{skipDuplicates:true});
