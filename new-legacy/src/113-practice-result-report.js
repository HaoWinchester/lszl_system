...
;(function (global) {
  ...
  function render(root, report, options = {}) {
    ...
    root.innerHTML = `<article class="practice-result-report">
      ...
      <section class="practice-report-overall ${resultClass}"><div><span>OVERALL PERFORMANCE</span><h2>${escapeHTML(report.resultLabel || `模拟考试结果：${report.passed ? 'PASS' : 'FAIL'}`)}</h2><p>本结果按本次会话的冻结规则生成。</p></div><div class="practice-report-score"><strong>${score.toFixed(2).replace(/\.00$/, '')}</strong><span>模拟分 / 100</span></div></section>
      <section class="practice-report-band-scale" aria-label="总体表现区间"><div class="practice-report-band-track">${Object.keys(BAND_LABELS).map(band => `<span class="practice-report-band-segment" style="--band-color:${BAND_COLORS[band]}">${BAND_LABELS[band]}</span>`).join('')}<i style="left:${Math.max(0, Math.min(100, score))}%" aria-label="你的模拟分 ${score}"></i></div></section>
      <section class="practice-report-counts"><div><span>总题数</span><strong>${number(counts.total)}</strong></div><div><span>答对</span><strong>${number(counts.correct)}</strong></div><div><span>答错</span><strong>${number(counts.wrong)}</strong></div><div><span>未答</span><strong>${number(counts.unanswered)}</strong></div><div><span>正确率</span><strong>${score.toFixed(2).replace(/\.00$/, '')}%</strong></div><div><span>累计用时</span><strong>${durationLabel(report.durationMs)}</strong></div></section>
