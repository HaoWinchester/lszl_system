...
(function(global){
  ...
  function attachmentHtml(item){const attachment=item.attachment;if(!attachment?.dataUrl)return '';const safe=/^data:image\/(png|jpeg|webp|gif);base64,/i.test(attachment.dataUrl)?attachment.dataUrl:'';return safe?`<p><a href="${safe}" target="_blank" rel="noopener">查看截图：${escapeHtml(attachment.name||'附件')}</a></p>`:''}
