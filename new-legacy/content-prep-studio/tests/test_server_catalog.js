...
async function testNonCommittedBatchNeverMutatesSyncMetadata(){
  const service=loadService(async(url)=>{
    if(url==='/api/v1/content-prep/batches')return response(200,{
      batchId:'batch-failed',bankId:'bank-1',bankRevision:9,
      questions:[{questionId:'q-1',status:'updated',revision:8,contentHash:'b'.repeat(64)}],
    });
    return response(200,{batch:{id:'batch-failed',status:'failed'}});
  });
  ...
  await assert.rejects(
    ...
    error=>error.code==='UPLOAD_NOT_COMMITTED',
  );
...
Promise.resolve()
  ...
  .then(testNonCommittedBatchNeverMutatesSyncMetadata)
