...
function renderQuestionPrincipleBindings(){
  ...
  box.innerHTML=`<div class="help">题目级原则保存到 <code>metadata.principleIds</code>；A/B/C/D 选项级映射保存到 <code>metadata.optionPrincipleMap</code>，用于“题目演示校验”和未来做题大厅。</div>
  <div style="margin-top:9px"><label>题目级原则</label>${principleCheckMarkup(q.metadata.principleIds,'question')}</div>
  ...
  box.querySelectorAll('[data-principle-check="question"]').forEach(el=>el.addEventListener('change',()=>{q.metadata.principleIds=[...box.querySelectorAll('[data-principle-check="question"]:checked')].map(x=>x.value);syncQuestionPrinciples(q);renderCurrentIssues()}));
...
function renderDemoPrinciples(q,optionId){
  const box=document.getElementById('demoPrincipleResult');if(!box)return;const mapped=q.metadata?.optionPrincipleMap?.[optionId]||[],ids=mapped.length?mapped:(q.metadata?.principleIds||[]);
...
function runValidation(){
  ...
  state.questionBank.questions.forEach(q=>{(q.metadata?.principleIds||[]).forEach(id=>{if(!principleById(id))issues.push({level:'error',object:q.id,message:`原则 ID 不存在：${id}`,suggest:'在原则管理中创建或重新绑定。'})});Object.entries(q.metadata?.optionPrincipleMap||{}).forEach(([opt,ids])=>(ids||[]).forEach(id=>{if(!principleById(id))issues.push({level:'error',object:q.id,message:`选项 ${opt} 的原则 ID 不存在：${id}`,suggest:'重新绑定选项原则。'})}));(q.tags||[]).forEach(tag=>{if(!tagPathFor(tag))issues.push({level:'warn',object:q.id,message:`标签“${tag}”未绑定主程序预设标签槽位`,suggest:'保留为自由标签，或在标签管理中映射/改名。'})})});
